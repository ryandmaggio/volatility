...
class NoneObject(object):
    ...
    def __format__(self, formatspec):
        spec = fmtspec.FormatSpec(
            string=formatspec, altform=False, formtype='s', fill="-", align=">"
        )
...
def Object(theType, offset, vm, name=None, **kwargs):
    ...
    try:
        if vm.profile.has_type(theType):
            result = vm.profile.types[theType](
                offset=offset, vm=vm, name=name, **kwargs
            )
            return result
...
class BaseObject(object):
    ...
    def rebase(self, offset):
        ...
        raise DeprecationWarning(
            "The rebase function has been deprecated and will be removed in future versions"
        )
    ...
    def __eq__(self, other):
        return self.v() == other or (
            ...
            and (self.obj_offset == other.obj_offset)
            and (self.obj_vm == other.obj_vm)
        )
    ...
    def dereference_as(self, derefType, **kwargs):
        ...
        if self.obj_native_vm.is_valid_address(self.v()):
            return Object(
                derefType, self.v(), self.obj_native_vm, parent=self, **kwargs
            )
        else:
            return NoneObject(
    ...
    def __init__(
        ...
        **kwargs
        ...
        format_string = vm.profile.native_types.get(
            native_type, vm.profile.native_types['address']
        )[1]
        NativeType.__init__(
            self, theType, offset, vm, format_string=format_string, **kwargs
        )
...
class CType(BaseObject):
    ...
    def __setattr__(self, attr, value):
        ...
        if (
            '_CType__initialized' not in self.__dict__
...
class VolatilityMagic(BaseObject):
    ...
    def __init__(
        self, theType, offset, vm, value=None, configname=None, **kwargs
    ):
        try:
            BaseObject.__init__(self, theType, offset, vm, **kwargs)
        except InvalidOffsetError:
...
class Profile(object):
    ...
    def add_types(self, vtypes, overlay=None):
        ...
        debug.warning(
            "Deprecation warning: A plugin is making use of profile.add_types"
        )
    ...
    def apply_overlay(self, *args, **kwargs):
        ...
        debug.warning(
            "Deprecation warning: A plugin is making use of profile.apply_overlay"
        )
        ...
        if isinstance(type_member, dict):
            ...
            for i in range(len(overlay)):
                if overlay[i] == None:
                    ...
                    result.append(
                        self._apply_overlay(type_member[i], overlay[i])
                    )
        else:
            return copy.deepcopy(overlay)
    ...
    def _resolve_mod_dependencies(self, mods):
        ...
        for mod in mods:
            ...
            data[mod.__class__.__name__] = data.get(
                mod.__class__.__name__, set([])
            ).union(set(before))
            for a in after:
                data[a] = data.get(a, set([])).union(
                    set([mod.__class__.__name__])
                )
        ...
        extra_items_in_deps = reduce(set.union, list(data.values())) - set(
            data.keys()
        )
    ...
    def _list_to_type(self, name, typeList, typeDict=None):
        ...
        debug.warning(
            "Unable to find a type for {0}, assuming int".format(typeList[0])
        )

...
config.add_option(
    ...
    help="Additional plugin directories to use ("
    + help_prefix
    + "colon separated)",
)

# Add the PLUGINPATH, in case we're frozen
__path__ = [constants.PLUGINPATH] + [
    e for e in __path__ if not constants.PLUGINPATH.startswith(e)
]
...
if config.PLUGINS:
    plugin_paths = [
        os.path.abspath(x) for x in config.PLUGINS.split(plugin_separator)
    ]

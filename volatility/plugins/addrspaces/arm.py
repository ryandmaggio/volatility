...
class ArmAddressSpace(paged.AbstractWritablePagedMemory):
    ...
    def pde2_value(self, vaddr, pde):
        return self.read_long_phys(
            (pde & 0xFFFFFC00) | (self.pde2_index(vaddr) << 2)
        )
    ...
    def pde2_value_fine(self, vaddr, pde):
        return self.read_long_phys(
            (pde & 0xFFFFF000) | (self.pde2_index_fine(vaddr) << 2)
        )
    ...
    def get_pte(self, vaddr, pde_value):
        ...
        if (pde_value & 0b11) == 0b00:
            ...
            if (pde2_value & 0b11) == 0b01:
                ...
                debug.warning(
                    "get_pte: invalid course pde2_value {0:x}".format(
                        pde2_value
                    )
                )
                return None
            ...
            if (pde2_value & 0b11) == 0b01:
                ...
                debug.warning(
                    "get_pte: invalid fine pde2_value {0:x}".format(pde2_value)
                )
                return None

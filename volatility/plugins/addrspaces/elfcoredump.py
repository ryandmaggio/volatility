...
class VirtualBoxModification(obj.ProfileModification):
    def modification(self, profile):
        ...
        profile.object_classes.update(
            {'DBGFCOREDESCRIPTOR': DBGFCOREDESCRIPTOR}
        )
...
class VirtualBoxCoreDumpElf64(addrspace.AbstractRunBasedMemory):
    ...
    def __init__(self, base, config, **kwargs):
        ...
        self.as_assert(
            str(elf.e_type) == 'ET_CORE', "ELF type is not a Core file"
        )
        ...
        for phdr in elf.program_headers():
            ...
            if str(phdr.p_type) == 'PT_NOTE':
                note = obj.Object(
                    "elf_note", offset=phdr.p_offset, vm=base, parent=phdr
                )
            ...
            if (
                str(phdr.p_type) != 'PT_LOAD'
                or phdr.p_filesz == 0
                or phdr.p_filesz != phdr.p_memsz
            ):
                continue

            self.runs.append(
                (int(phdr.p_paddr), int(phdr.p_offset), int(phdr.p_memsz))
            )
    ...
    def validate(self):
        self.as_assert(
            self.header,
            'ELF error: did not find any PT_NOTE segment with VBCORE',
        )
        self.as_assert(
            self.header.u32Magic == DBGFCORE_MAGIC,
            'Could not find VBox core magic signature',
        )
        self.as_assert(
            self.header.u32FmtVersion & 0xFFFFFFF0 == DBGFCORE_FMT_VERSION,
            'Unknown VBox core format version',
        )
        self.as_assert(
            self.runs, 'ELF error: did not find any LOAD segment with main RAM'
        )
...
class QemuCoreDumpElf(VirtualBoxCoreDumpElf64):
    ...
    def validate(self):
        self.as_assert(
            self.header,
            'ELF error: did not find any PT_NOTE segment with CORE',
        )
        self.as_assert(
            self.runs, 'ELF error: did not find any LOAD segment with main RAM'
        )

...
class VMWareMetaAddressSpace(addrspace.AbstractRunBasedMemory):
    ...
    def __init__(self, base, config, **kwargs):
        ...
        base_vmem = (
            hasattr(base, 'vmem_address_space') and base.vmem_address_space
        )

        self.as_assert(not base_vmem, "Can not stack over another vmem")

        base_page = (
            hasattr(base, 'paging_address_space') and base.paging_address_space
        )

        self.as_assert(
            not base_page, "Can not stack over another paging address space"
        )
        ...
        self.as_assert(
            location != metadata, 'VMware metadata file already detected'
        )
        ...
        self.as_assert(
            header.Magic in [0xBED2BED0, 0xBAD1BAD1, 0xBED2BED2, 0xBED3BED3],

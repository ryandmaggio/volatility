...
class _VMWARE_TAG(obj.CType):
    ...
    def OriginalDataOffset(self):
        ...
        return (
            self.Name.obj_offset
            + self.NameLength
            + (
                self.TagIndices.count
                * self.obj_vm.profile.get_obj_size("unsigned int")
            )
        )
    ...
    def RealDataOffset(self):
        ...
        if self.OriginalDataSize in (62, 63):
            ...
            padlen = obj.Object(
                "unsigned short", offset=offset, vm=self.obj_vm
            )
    ...
    def cast_as(self, cast_type):
        ...
        return obj.Object(
            cast_type, offset=self.RealDataOffset, vm=self.obj_vm
        )
        ...
        profile.vtypes.update(
            {
                ...
                '_VMWARE_GROUP': [
                    80,
                    {
                        'Name': [
                            0,
                            ['String', dict(length=64, encoding='utf8')],
                        ],
                ...
                '_VMWARE_TAG': [
                    None,
                    {
                        ...
                        'TagIndices': [
                            lambda x: x.obj_offset + 2 + x.NameLength,
                            [
                                'array',
                                lambda x: (x.Flags >> 6) & 0x3,
                                ['unsigned int'],
                            ],
                        ],
                    },
...
class VMWareAddressSpace(addrspace.AbstractRunBasedMemory):
    ...
    def __init__(self, base, config, **kwargs):
        ...
        if not region_count.is_valid() or region_count == 0:
            ...
            memory_tag = self.get_tag(
                self.header, grp_name="memory", tag_name="Memory"
            )

            self.as_assert(
                memory_tag != None, "Cannot find the single-region Memory tag"
            )
            ...
            for i in range(region_count):
                ...
                self.as_assert(
                    memory_tag != None, "Cannot find the Memory tag"
                )
    ...
    def get_tag(header, grp_name, tag_name, indices=None, data_type=None):
        ...
        return obj.NoneObject(
            "Cannot find [{0}][{1}]".format(grp_name, tag_name)
        )

...
class BigPageTable(obj.VolatilityMagic):
    ...
    def generate_suggestions(self):
        ...
        for pair in self.distance:
            table_base = obj.Object(
                "address", offset=track_table - pair[0], vm=self.obj_vm
            )

            table_size = obj.Object(
                "address", offset=track_table - pair[1], vm=self.obj_vm
            )
...
class BigPagePoolScanner(object):
    ...
    def scan(self, tags=[]):
        ...
        (table_base, table_size) = obj.VolMagic(
            self.kernel_space
        ).BigPageTable.v()

        pools = obj.Object(
            'Array',
            targetType='_POOL_TRACKER_BIG_PAGES',
            offset=table_base,
            count=table_size,

...
class _CONTROL_AREA(obj.CType):
    def extract_ca_file(self, unsafe=False):
        ...
        # PTE entries.
        memory_model = self.obj_vm.profile.metadata.get(
            'memory_model', '32bit'
        )
        pae = self.obj_vm.pae
        ...
        while subsection.is_valid() and subsection.v() != 0x0:
            ...
            while ptecount < PtesInSubsection:
                ...
                if pae:
                    objname = "_MMPTEPA"
                mmpte = obj.Object(
                    objname, offset=pteoffset, vm=subsection.obj_vm
                )
...
class _SHARED_CACHE_MAP(obj.CType):
    def is_valid(self):
        ...
        if SectionSize < 0 or (
            (FileSize < ValidDataLength)
            and (ValidDataLength != 0x7FFFFFFFFFFFFFFF)
        ):
            return False
    ...
    def process_index_array(self, array_pointer, level, limit, vacbary=None):
        ...
        for _i in range(0, VACB_ARRAY):
            ...
            Vacbs = obj.Object(
                "_VACB", offset=int(VacbArray[_i]), vm=self.obj_vm
            )
            ...
            if Vacbs.SharedCacheMap == self.obj_offset:
                ...
                vacbary = self.process_index_array(
                    VacbArray[_i], level + 1, limit, vacbary
                )
    ...
    def extract_scm_file(self):
        ...
        if not SectionSize > VACB_SIZE_OF_FIRST_LEVEL:
            ...
            for _i in range(0, full_blocks):
                ...
                vacb_entry = obj.Object(
                    "address", offset=vacb_addr, vm=Vacbs.obj_vm
                )
                ...
                if not vacb_entry or (vacb_entry.v() == 0x0):
                    continue
                Vacb = obj.Object(
                    "_VACB", offset=vacb_entry.v(), vm=self.obj_vm
                )
        ...
        if SectionSize > VACB_SIZE_OF_FIRST_LEVEL:
            ...
            for _i in range(0, VACB_ARRAY):
                ...
                if Vacb.SharedCacheMap == self.obj_offset:
                    ...
                    # current level to 2.
                    vacbary = self.process_index_array(
                        VacbArray[_i], 2, limit_depth, vacbary
                    )
                    # vacbary = vacbary + _vacbary
    ...
    '_MMPTE_SUBSECTION_64': [
        0x8,
        {
            ...
            'SubsectionAddress': [
                ...
                [
                    'BitField',
                    dict(start_bit=32, end_bit=64, native_type='long long'),
                ],
            ],
        },
    ],
    '_MMPTE_TRANSITION_64': [
        0x8,
        {
            'Valid': [
                0x0,
            ...
            try:
                ...
                for p in self._config.PHYSOFFSET.split(","):
                    file_obj = obj.Object(
                        "_FILE_OBJECT",
                        int(p, 16),
                        self.kaddr_space.base,
                        native_vm=self.kaddr_space,
                    )
                ...
                if not self.filters or "HandleTable" in self.filters:
                    ...
                    if task.ObjectTable.HandleTableList:
                        for handle in task.ObjectTable.handles():
                            ...
                            if otype == "File":
                                file_obj = handle.dereference_as(
                                    "_FILE_OBJECT"
                                )
                                ...
                                if file_obj:
                                    ...
                                    if self._config.REGEX:
                                        ...
                                        if file_obj.FileName:
                                            name = str(
                                                file_obj.file_name_with_device()
                                            )
                                        if not name:
                                            continue
                                        if not file_re.search(name):
                ...
                if file_obj.SectionObjectPointer:
                    ...
                    if (
                        ...
                        if ImageSectionObject and ImageSectionObject != 0:
                            ...
                            if not control_area in control_area_list:
                                ...
                                (mdata, zpad) = control_area.extract_ca_file(
                                    self._config.UNSAFE
                                )
                    ...
                    if not self.filters or "DataSectionObject" in self.filters:
                        ...
                        if DataSectionObject and DataSectionObject != 0:
                            ...
                            control_area = DataSectionObject.dereference_as(
                                '_CONTROL_AREA'
                            )
                    ...
                    if SharedCacheMap:
                        ...
                        shared_cache_map = SharedCacheMap.dereference_as(
                            '_SHARED_CACHE_MAP'
                        )
        ...
        for summaryinfo in data:
            if summaryinfo['type'] == "DataSectionObject":
                ...
                for vacb in summaryinfo['vacbary']:
                    if not vacb:
                        continue
                    (rdata, mdata, zpad) = self.audited_read_bytes(
                        self.kaddr_space, vacb['baseaddr'], vacb['size'], True
                    )

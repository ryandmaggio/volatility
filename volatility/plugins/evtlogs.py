    ...
    'EVTRecordStruct': [
        0x38,
        {
            ...
            'NumStrings': [
                0x1A,
                ['unsigned short'],
            ],  # number of description strings in even message
...
class EvtLogs(common.AbstractWindowsCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        config.add_option(
            ...
            default=False,
            action='store_true',
            help='Save the raw .evt files also',
        )
    ...
    def get_sid_string(self, data):
        ...
        for i in sid.IdentifierAuthority.Value:
            id_auth = i
        sid_string = "S-" + "-".join(
            str(i) for i in (sid.Revision, id_auth) + tuple(sid.SubAuthority)
        )
        if sid_string in getsids.well_known_sids:
            sid_name = " ({0})".format(getsids.well_known_sids[sid_string])
        else:
            sid_name_re = getsids.find_sid_re(
                sid_string, getsids.well_known_sid_re
            )
            if sid_name_re:
    ...
    def parse_evt_info(self, name, buf, rawtime=False):
        ...
        while loc != -1:
            ...
            if evtlog.SidLength == 0:
                ...
                sid_string = self.get_sid_string(
                    rec[end : end + evtlog.SidLength]
                )
            ...
            strings = rec[evtlog.StringOffset :].split(
                "\x00\x00", evtlog.NumStrings
            )
            messages = []

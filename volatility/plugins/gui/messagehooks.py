...
class MessageHooks(atoms.Atoms, sessions.SessionsMixin):
    ...
    def calculate(self):
        ...
        atom_tables = dict(
            (atom_table, winsta)
            for (atom_table, winsta) in atoms.Atoms(self._config).calculate()
        )

        # Unique window stations
        window_stations = [
            winsta for winsta in list(atom_tables.values()) if winsta
        ]
    ...
    def translate_atom(self, winsta, atom_tables, atom_id):
        ...
        table_list = [
            table
            for (table, window_station) in list(atom_tables.items())
            if window_station == None
        ]
    ...
    def translate_hmod(self, winsta, atom_tables, index):
        ...
        if (
            winsta.obj_vm.profile.metadata.get('memory_model', '32bit')
            == '32bit'
        ):

        ...
        for proc in tasks.pslist(kernel_space):
            if proc.SessionId != None and proc.SessionId.v() not in seen:
                ...
                if ps_ad != None:
                    ...
                    yield obj.Object(
                        "_MM_SESSION_SPACE", offset=proc.Session.v(), vm=ps_ad
                    )
...
class Sessions(common.AbstractWindowsCommand, SessionsMixin):
    ...
    def render_text(self, outfd, data):
        ...
        mods = dict(
            (kernel_space.address_mask(mod.DllBase), mod)
        ...
        for session in data:
            outfd.write("*" * 50 + "\n")
            outfd.write(
                "Session(V): {0:x} ID: {1} Processes: {2}\n".format(
                    session.obj_offset,
                    session.SessionId,
            ...
            outfd.write(
                "PagedPoolStart: {0:x} PagedPoolEnd {1:x}\n".format(

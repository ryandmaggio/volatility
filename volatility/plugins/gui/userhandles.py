        ...
        for session in data:
            ...
            outfd.write(
                "aheList: {0:#x}, Table size: {1:#x}, Entry size: {2:#x}\n".format(
                    ...
                    shared_info.HeEntrySize
                    if hasattr(shared_info, 'HeEntrySize')
                    else shared_info.obj_vm.profile.get_obj_size(
                        "_HANDLEENTRY"
                    ),
                )
            )
            ...
            if self._config.PID:
                filters.append(
                    lambda x: x.Process.UniqueProcessId == self._config.PID
                )

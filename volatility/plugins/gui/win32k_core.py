...
class _MM_SESSION_SPACE(obj.CType):
    ...
    def _section_chunks(self, sec_name):
        ...
        dos_header = obj.Object(
            "_IMAGE_DOS_HEADER", offset=self.Win32KBase, vm=self.obj_vm
        )
        ...
        buffer_as = addrspace.BufferAddressSpace(
            conf.ConfObject(), data=data, base_offset=self.Win32KBase
        )
    ...
    def find_gahti(self):
        ...
        for chunk in self._section_chunks(".rdata"):
            ...
            gahti = obj.Object(
                "gahti", offset=chunk.obj_offset, vm=self.obj_vm
            )
    ...
    def find_shared_info(self):
        ...
        for chunk in self._section_chunks(".data"):
            ...
            shared_info = obj.Object(
                "tagSHAREDINFO", offset=chunk.obj_offset, vm=self.obj_vm
            )
            # Sanity check it
            try:
...
class _HANDLEENTRY(obj.CType):
    ...
    def Thread(self):
        ...
        if self.ThreadOwned:
            return self.pOwner.dereference_as(
                "tagTHREADINFO"
...
class tagWND(obj.CType):
    ...
    def _get_flags(self, member, flags):
        ...
        return ','.join(
            [n for (n, v) in list(flags.items()) if member & v == v]
        )
...
class tagEVENTHOOK(obj.CType):
    ...
    def dwFlags(self):
        ...
        flags = [
            name
            for (val, name) in list(consts.EVENT_FLAGS.items())
            if f & val == val
        ]
...
class _RTL_ATOM_TABLE(tagWINDOWSTATION):
    ...
    def is_valid(self):
        ...
        return (
            ...
            and self.Signature == 0x6D6F7441
            and self.NumBuckets < 0xFFFF
        )
    ...
    def NumBuckets(self):
        ...
        number = obj.Object(
            "unsigned long",
            offset=self.obj_offset + offset + 0x40,
            vm=self.obj_vm,
        )
        ...
        profile.vtypes.update(
            {
                'gahti': [
                    None,
                    {
                        'types': [
                            0,
                            ['array', num_handles, ['tagHANDLETYPEINFO']],
                        ],
                    },
                ]
            }
        ...
        profile.vtypes.update(
            {
                'tagTIMER': [
                    None,
                    {
                        ...
                        'flags': [
                            0x24,
                            ['Flags', {'bitmap': consts.TIMER_FLAGS}],
                        ],
        ...
        profile.vtypes.update(
            {
                ...
                'tagCLIPDATA': [
                    None,
                    {
                        ...
                        'abData': [
                            0x0C,
                            ['array', lambda x: x.cbData, ['unsigned char']],
                        ],
                    },
                ],

        ...
        for winsta, atom_tables in data:
            for desktop in winsta.desktops():
                ...
                for wnd, _level in desktop.windows(desktop.DeskInfo.spwnd):
                    ...
                    if (
                        wanted_pids
                        and not wnd.Process.UniqueProcessId in wanted_pids
                    ):
                        continue

                    outfd.write(
                        "Window Handle: #{0:x} at {1:#x}, Name: {2}\n".format(
                            wnd.head.h, wnd.obj_offset, str(wnd.strName or '')
                        )
                    )
                    outfd.write(
                        "ClassAtom: {0:#x}, Class: {1}\n".format(
                            wnd.ClassAtom,
                            self.translate_atom(
                                winsta, atom_tables, wnd.ClassAtom
                            ),
                    ...
                    outfd.write(
                        "SuperClassAtom: {0:#x}, SuperClass: {1}\n".format(
                            wnd.SuperClassAtom,
                            self.translate_atom(
                                winsta, atom_tables, wnd.SuperClassAtom
                            ),
                        )
                    )
                    outfd.write(
                        "pti: {0:#x}, Tid: {1} at {2:#x}\n".format(
                            wnd.head.pti.v(),

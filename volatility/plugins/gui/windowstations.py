...
class PoolScanWind(poolscan.PoolScanner):
    ...
    def __init__(self, address_space):
        ...
        size = (
            0x90  # self.address_space.profile.get_obj_size("tagWINDOWSTATION")
        )
...
class WndScan(common.AbstractScanCommand, sessions.SessionsMixin):
    ...
    def calculate(self):
        ...
        for wind in self.scan_results(addr_space):
            ...
            for winsta in wind.traverse():
                if (
                    winsta.is_valid()
                    and len([desk for desk in winsta.desktops()]) > 0
                ):
    ...
    def render_text(self, outfd, data):
        ...
        for window_station in data:
            ...
            outfd.write(
                "SessionId: {0}, AtomTable: {1:#x}, Interactive: {2}\n".format(
            ...
            outfd.write(
                "spwndClipOpen: {0:#x}, spwndClipViewer: {1:#x} {2} {3}\n".format(
                    window_station.spwndClipOpen.v(),
                    window_station.spwndClipViewer.v(),
                    str(
                        window_station.LastRegisteredViewer.UniqueProcessId
                        or ""
                    ),
                    str(
                        window_station.LastRegisteredViewer.ImageFileName or ""
                    ),
                )
            )
            outfd.write(
                "cNumClipFormats: {0}, iClipSerialNumber: {1}\n".format(
                    window_station.cNumClipFormats,
                    window_station.iClipSerialNumber,
                )
            )
            outfd.write(
                "pClipBase: {0:#x}, Formats: {1}\n".format(
                    window_station.pClipBase,
                    ",".join(
                        [
                            str(clip.fmt)
                            for clip in window_station.pClipBase.dereference()
                        ]
                    ),
                )
            )

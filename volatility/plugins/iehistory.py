...
class _URL_RECORD(obj.CType):
    ...
    def is_valid(self):
        ...
        if (
            ...
            and self.Length > 0
            and self.Length < 32768
        ):
            if (
                not str(self.LastModified).startswith("1970-01-01")
                and str(self.LastModified) != "-"
            ):
                if (
                    not str(self.LastAccessed).startswith("1970-01-01")
                    and str(self.LastAccessed) != "-"
                ):
                    ret = True
    ...
    def has_data(self):
        ...
        return (
            self.DataOffset > 0
            and self.DataOffset < self.Length
...
class _DEST_RECORD(obj.CType):
    def is_valid(self):
        ...
        if (
            obj.CType.is_valid(self)
            and self.LastModified.is_valid()
            and self.LastAccessed.is_valid()
        ):
            if (
                not str(self.LastModified).startswith("1970-01-01")
                and str(self.LastModified) != "-"
                ...
                if (
                    ...
                    if (
                        1999 < self.LastModified.as_datetime().year < 2075
                        and 1999 < self.LastAccessed.as_datetime().year < 2075
                        and self.URLStart.is_valid()
                    ):
                        ret = True
        return ret
        ...
        profile.vtypes.update(
            {
                '_URL_RECORD': [
                    None,
                    {
                        ...
                        'Url': [
                            lambda x: x.obj_offset + x.UrlOffset,
                            ['String', dict(length=4096)],
                        ],
                        'File': [
                            lambda x: x.obj_offset + x.FileOffset,
                            ['String', dict(length=4096)],
                        ],
                        'Data': [
                            lambda x: x.obj_offset + x.DataOffset,
                            ['String', dict(length=4096)],
                        ],
                    },
                ],
...
class IEHistory(taskmods.DllList):
    ...
    def calculate(self):
        ...
        vad_filter = lambda x: (
            hasattr(x, 'ControlArea')
            and str(x.FileObject.FileName or '').endswith("index.dat")
        ) or (x.VadFlags.Protection.v() == 4)

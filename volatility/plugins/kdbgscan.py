...
class MultiStringFinderCheck(scan.ScannerCheck):
    ...
    def __init__(self, address_space, needles=None):
        ...
        if not self.maxlen:
            raise RuntimeError(
                "No needles of any length were found for the "
...
class KDBGScan(common.AbstractWindowsCommand):
    ...
    @cache.CacheDecorator(
        lambda self: "tests/kdbgscan/kdbg={0}".format(self._config.KDBG)
    )
    def calculate(self):
            ...
            for offset in scanner.scan(aspace):
                ...
                for l in proflens:
                    if val.find(proflens[l]) >= 0:
                        kdbg = obj.Object(
                            "_KDDEBUGGER_DATA64", offset=offset, vm=aspace
                        )
            ...
            for p, k in suspects:
                ...
                for offset in scanner.scan(nspace):
                    ...
                    if val.find(proflens[p]) >= 0:
                        kdbg = obj.Object(
                            "_KDDEBUGGER_DATA64", offset=offset, vm=nspace
                        )
                        yield p, kdbg
            self._config.update('PROFILE', origprofile)
    ...
    def render_text(self, outfd, data):
        ...
        for profile, kdbg in data:
            ...
            if hasattr(kdbg, 'KdCopyDataBlock'):
                outfd.write(
                    "{0:<30}: {1:#x}\n".format(
                        "KdCopyDataBlock (V)", kdbg.KdCopyDataBlock
                    )
                )
            ...
            outfd.write(
                "{0:<30}: {1}\n".format(
                    "KDBG owner tag check", str(kdbg.is_valid())
                )
            )
            outfd.write(
                "{0:<30}: {1}\n".format(

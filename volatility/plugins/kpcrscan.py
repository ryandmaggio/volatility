    ...
    def render_text(self, outfd, data):
        ...
        for kpcr in data:
            ...
            if hasattr(kpcr.obj_vm, 'vtop'):
                ...
                outfd.write(
                    "{0:<30}: {1:#x}\n".format(
                        "Offset (P)", kpcr.obj_vm.vtop(kpcr.obj_offset)
                    )
                )
            else:
                outfd.write(
            ...
            current_thread = kpcr.ProcessorBlock.CurrentThread.dereference_as(
                "_ETHREAD"
            )
            idle_thread = kpcr.ProcessorBlock.IdleThread.dereference_as(
                "_ETHREAD"
            )
            next_thread = kpcr.ProcessorBlock.NextThread.dereference_as(
                "_ETHREAD"
            )
            ...
            if current_thread:
                outfd.write(
                    "{0:<30}: {1:#x} TID {2} ({3}:{4})\n".format(
                        "CurrentThread",
                        current_thread.obj_offset,
                        current_thread.Cid.UniqueThread,
            ...
            if idle_thread:
                outfd.write(
                    "{0:<30}: {1:#x} TID {2} ({3}:{4})\n".format(
                        "IdleThread",
                        idle_thread.obj_offset,
                        idle_thread.Cid.UniqueThread,
...
class KPCRScannerCheck(scan.ScannerCheck):
    ...
    def check(self, offset):
        ...
        try:
            pSelfPCR = obj.Object(
                'Pointer',
                offset=(offset + self.SelfPcr_offset),
                vm=self.address_space,
            )
            pPrcb = obj.Object(
                'Pointer',
                offset=(offset + self.Prcb_offset),
                vm=self.address_space,
            )
            if self.address_equality(
                pSelfPCR, paKCPR
            ) and self.address_equality(pPrcb, paPRCBDATA):
                self.KPCR = pSelfPCR
                return True
...
class KPCRScanner(scan.BaseScanner):
    ...
    def scan(self, address_space, offset=0, maxlen=None):
        return scan.BaseScanner.scan(
            self, address_space, max(offset, 0x80000000), maxlen
        )

...
class linux_check_fop(linux_common.AbstractLinuxCommand):
    ...
    def check_file_cache(self, f_op_members, modules):
        for (_, _, file_path, file_dentry) in find_file.linux_find_file(
            ...
            for (hooked_member, hook_address) in self.verify_ops(
    ...
    def check_open_files_fop(self, f_op_members, modules):
        ...
        for task in tasks:
            ...
            for filp, i in task.lsof():
                for (hooked_member, hook_address) in self.verify_ops(
                    filp.f_op, f_op_members, modules
                ):
        ...
        for proc_mnt in proc_mnts:
            ...
            for dentry in root.d_subdirs.list_of_type("dentry", walk_member):
                ...
                for (hooked_member, hook_address) in self.verify_ops(
                    ...
                    yield (
                        "proc_mnt: {0:x}:{1}".format(root.v(), name),
                        hooked_member,
                        hook_address,
                    )
    ...
    def _walk_proc_old(self, cur, f_op_members, modules, parent):
        ...
        while cur:
            ...
            for (hooked_member, hook_address) in self.verify_ops(
                cur.proc_fops, f_op_members, modules
            ):
                yield (name, hooked_member, hook_address)
    ...
    def _walk_rb(self, rb):
        ...
        rboff = self.addr_space.profile.get_obj_offset(
            "proc_dir_entry", "subdir_node"
        )
        pde = obj.Object(
            "proc_dir_entry", offset=rb.v() - rboff, vm=self.addr_space
        )
    ...
    def _do_walk_proc_current(self, cur, f_op_members, modules, parent):
        ...
        for pde in self._walk_rb(cur.subdir.rb_node):
            ...
            nodes = nodes + self._do_walk_proc_current(
                pde, f_op_members, modules, name
            )

        return nodes

    def _walk_proc_current(self, cur, f_op_members, modules, parent):
        proc_entries = self._do_walk_proc_current(
            cur, f_op_members, modules, parent
        )

        for (pde, name) in proc_entries:
            for (hooked_member, hook_address) in self.verify_ops(
                pde.proc_fops, f_op_members, modules
            ):
                yield (name, hooked_member, hook_address)

    def _walk_proc_dir(self, proc_root, f_op_members, modules, parent):
        if self.addr_space.profile.obj_has_member(
            "proc_dir_entry", "subdir_node"
        ):
        ...
        for (name, hooked_member, hook_address) in walk_proc(
            proc_root, f_op_members, modules, parent
        ):
            yield (name, hooked_member, hook_address)
    ...
    def check_proc_root_fops(self, f_op_members, modules):
        ...
        for (name, hooked_member, hook_address) in self._walk_proc_dir(
            proc_root, f_op_members, modules, "/proc"
        ):
            yield (name, hooked_member, hook_address)
    ...
    def check_proc_net_fops(self, f_op_members, modules):
        ...
        for net in nethead.list_of_type("net", "list"):
            for (name, hooked_member, hook_address) in self._walk_proc_dir(
                net.proc_net, f_op_members, modules, "/proc/net"
            ):
                yield (name, hooked_member, hook_address)
    ...
    def calculate(self):
        ...
        if self._config.INODE:
            inode = obj.Object(
                "inode", offset=self._config.INODE, vm=self.addr_space
            )
            ...
            for (hooked_member, hook_address) in self.verify_ops(
                inode.i_fop, f_op_members, modules
            ):
                yield (
                    "inode at {0:x}".format(inode.obj_offset),
                    hooked_member,
                    hook_address,
                )
    ...
    def render_text(self, outfd, data):
        self.table_header(
            outfd,
            [("Symbol Name", "42"), ("Member", "30"), ("Address", "[addr]")],
        )

...
class linux_check_inline_kernel(linux_common.AbstractLinuxCommand):
    ...
    def _is_hooked(self, sym_addr, modules):
        ...
        for op in distorm3.Decompose(sym_addr, data, mode):
            ...
            if op.mnemonic == "JMP" and not self.is_known_address(
                op.operands[0].value, modules
            ):
                hook_type = "JMP"
            ...
            elif op.mnemonic == "CALL" and not self.is_known_address(
                op.operands[0].value, modules
            ):
                hook_type = "CALL"
    ...
    def check_file_cache(self, f_op_members, modules):
        for (_, _, file_path, file_dentry) in find_file.linux_find_file(
            ...
            for (
                ...
                hook_type,
                hook_address,
            ) in self._is_inline_hooked(
                file_dentry.d_inode.i_fop, f_op_members, modules
            ):
        ...
        for task in tasks:
            for filp, i in task.lsof():
                for (
                    ...
                    hook_type,
                    hook_address,
                ) in self._is_inline_hooked(filp.f_op, f_op_members, modules):
                    name = "{0:s} {1:d} {2:s}".format(
                        task.comm, i, linux_common.get_path(task, filp)
                    )
    ...
    def check_proc_fop(self, f_op_members, modules):
        ...
        proc_mnt_ptr = obj.Object(
            "Pointer", offset=proc_mnt_addr, vm=self.addr_space
        )
        proc_mnt = proc_mnt_ptr.dereference_as("vfsmount")

        root = proc_mnt.mnt_root

        for (hooked_member, hook_type, hook_address) in self._is_inline_hooked(
            root.d_inode.i_fop, f_op_members, modules
        ):
        ...
        for dentry in root.d_subdirs.list_of_type("dentry", "d_u"):
            ...
            for (
                ...
                dentry.d_inode.i_fop, f_op_members, modules
            ):
                yield (
                    "proc_mnt: {0}".format(name),
                    hooked_member,
                    hook_type,
                    hook_address,
                )
    ...
    def walk_proc(self, cur, f_op_members, modules, parent=""):
        while cur:
            ...
            for (
                ...
                hook_type,
                hook_address,
            ) in self._is_inline_hooked(fops, f_op_members, modules):
                yield (name, hooked_member, hook_type, hook_address)
    ...
    def check_proc_root_fops(self, f_op_members, modules):
        ...
        proc_root = obj.Object(
            "proc_dir_entry", offset=proc_root_addr, vm=self.addr_space
        )

        for (hooked_member, hook_type, hook_address) in self._is_inline_hooked(
            proc_root.proc_fops, f_op_members, modules
        ):
            yield ("proc_root", hooked_member, hook_type, hook_address)

        for (name, hooked_member, hook_type, hook_address) in self.walk_proc(
            proc_root, f_op_members, modules
        ):
            yield (name, hooked_member, hook_type, hook_address)
    ...
    def _check_file_op_pointers(self, modules):
        ...
        f_op_members = list(
            self.profile.types['file_operations'].keywords["members"].keys()
        )
        f_op_members.remove('owner')

        for func in funcs:
            for (name, member, hook_type, address) in func(
    ...
    def check_afinfo(self, var_name, var, op_members, seq_members, modules):
        for (hooked_member, hook_type, hook_address) in self._is_inline_hooked(
            var.seq_fops, op_members, modules
        ):
        ...
        if hasattr(var, "seq_ops"):
            for (
                ...
                hook_type,
                hook_address,
            ) in self._is_inline_hooked(var.seq_ops, seq_members, modules):
                yield (var_name, hooked_member, hook_type, hook_address)
    ...
    def _check_afinfo(self, modules):
        ...
        seq_members = list(
            self.profile.types['seq_operations'].keywords["members"].keys()
        )
        ...
        for proto in protocols:
            ...
            for global_var_name in proto[1]:
                global_var_addr = self.addr_space.profile.get_symbol(
                    global_var_name
                )
                ...
                global_var = obj.Object(
                    struct_type, offset=global_var_addr, vm=self.addr_space
                )

                for (name, member, hook_type, address) in self.check_afinfo(
                    global_var_name,
                    global_var,
                    op_members,
                    seq_members,
    ...
    def _check_inetsw(self, modules):
        ...
        proto_members = list(
            self.profile.types['proto_ops'].keywords["members"].keys()
        )
        ...
        for inet_list in inetsw:
            for inet in inet_list.list_of_type("inet_protosw", "list"):
                ...
                for (
                    ...
                    hook_type,
                    hook_address,
                ) in self._is_inline_hooked(inet.ops, proto_members, modules):
                    yield (name, hooked_member, hook_type, hook_address)
    ...
    def calculate(self):
        ...
        if not has_distorm3:
            debug.error(
                "This plugin cannot operate without distrom installed."
            )

        modules = linux_lsmod.linux_lsmod(self._config).get_modules()

        funcs = [
            self._check_known_functions,
            self._check_file_op_pointers,
            self._check_afinfo,
            self._check_inetsw,
        ]

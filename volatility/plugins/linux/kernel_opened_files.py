...
class linux_kernel_opened_files(linux_common.AbstractLinuxCommand):
    ...
    def _walk_node_hash(self, node):
        ...
        hash_offset = self.addr_space.profile.get_obj_offset(
            "dentry", "d_hash"
        )
        while node.is_valid() and node != last_node:
            ...
            dentry = obj.Object(
                "dentry", offset=node.v() - hash_offset, vm=self.addr_space
            )
            cnt = cnt + 1
            last_node = node
    ...
    def _gather_dcache(self):
        d_hash_shift = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("d_hash_shift"),
            vm=self.addr_space,
        )
        loop_max = 1 << d_hash_shift

        d_htable_ptr = obj.Object(
            "Pointer",
            offset=self.addr_space.profile.get_symbol("dentry_hashtable"),
            vm=self.addr_space,
        )

        arr = obj.Object(
            theType="Array",
            targetType="hlist_bl_head",
            offset=d_htable_ptr,
            vm=self.addr_space,
            count=loop_max,
        )
    ...
    def calculate(self):
        ...
        for dentry_offset in self._compare_filps():
            dentry = obj.Object(
                "dentry", offset=dentry_offset, vm=self.addr_space
            )
            if (
                dentry.d_count > 0
                and dentry.d_inode.is_reg()
                and dentry.d_flags == 128
            ):
                yield dentry
    ...
    def generator(self, data):
        for dentry in data:
            yield (
                0,
                Address(dentry.obj_offset),
                str(dentry.get_partial_path()),
            )

    def unified_output(self, data):
        return TreeGrid(
            [("Offset (V)", Address), ("Partial File Path", str)],
            self.generator(data),
        )
    ...
    def render_text(self, outfd, data):
        ...
        self.table_header(
            outfd, [("Offset (V)", "[addrpad]"), ("Partial File Path", "")]
        )
        for dentry in data:

...
class PassphraseScanner(malfind.BaseYaraScanner):
    ...
    def scan(self, offset=0, maxlen=None):
        ...
        for vma in self.task.get_proc_maps():
            ...
            if not (
                vma.vm_start <= self.task.mm.start_brk
                and vma.vm_end >= self.task.mm.brk
            ):
                continue

            for hit, address in malfind.BaseYaraScanner.scan(
                self, vma.vm_start, vma.vm_end - vma.vm_start
            ):
                ...
                if (
                    passt
                    and vma.vm_start <= passt.Text
                    and vma.vm_end >= passt.Text
                    and passt.Length > 0
                    and passt.Length < passt.MaxLength
                ):

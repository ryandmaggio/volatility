...
class linux_volshell(volshell.volshell):
    ...
    def modules(self):
        ...
        for (module, _, __) in mods:
            print(
                "{0:24} {1:d}".format(
                    module.name, module.init_size + module.core_size
                )
            )
    ...
    def ps(self, procs=None):
        ...
        for proc in procs or self.getpidlist():
            print(
                "{0:16} {1:<6} {2:#08x}".format(
                    proc.comm, proc.pid, proc.obj_offset
                )
            )
    ...
    def context_display(self):
        ...
        print(
            "Current context: process {0}, pid={1} DTB={2:#x}".format(
                self._proc.comm, self._proc.pid, dtb
            )
        )

    def set_context(self, offset=None, pid=None, name=None, physical=False):
        if physical and offset != None:
            offset = pslist.linux_pslist.virtual_process_from_physical_offset(
        ...
        self._proc = obj.Object(
            "task_struct", offset=offset, vm=self._addrspace
        )

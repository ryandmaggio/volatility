...
class linux_lsmod(linux_common.AbstractLinuxCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self._config.add_option(
            ...
            short_option='c',
            default=None,
            help='Path to IDC file to be created for module',
            action='store',
            type='str',
        )
    ...
    def _get_modules(self):
        if self._config.BASE:
            ...
            yield obj.Object(
                "module", offset=module_address, vm=self.addr_space
            )
        else:
            modules_addr = self.addr_space.profile.get_symbol("modules")

            modules = obj.Object(
                "list_head", vm=self.addr_space, offset=modules_addr
            )
        ...
        for module in self._get_modules():
            if self._config.PARAMS:
                if not hasattr(module, "kp"):
                    debug.error(
                        "Gathering module parameters is not supported in this profile."
                    )
...
class linux_moddump(linux_common.AbstractLinuxCommand):
    ...
    def _get_header_64(self, load_addr, sect_hdr_offset, num_sects):
        ...
        e_shnum = struct.pack(
            "<H", num_sects + 1
        )  # this works as we stick the seciton we create at the end
    ...
    def _build_sections_list(self, module):
        ...
        if symtab_idx == -1:
            debug.error(
                "No section .symtab found. Unable to properly re-create ELF file."
            )
    ...
    def _parse_sections(self, module):
        ...
        for (i, (name, address)) in enumerate(orig_sections):
            ...
            updated_sections.append(
                (name, address, size, sect_bytes, section_data)
            )
    ...
    def _make_sect_header_64(
        self, name, address, size, file_off, strtab_idx, symtab_idx
    ):
        ...
        sh_link = struct.pack(
            "<I", self._calc_link(name, strtab_idx, symtab_idx, int_sh_type)
        )
        sh_info = "\x00" * 4
        sh_addralign = "\x01\x00\x00\x00\x00\x00\x00\x00"
        sh_entsize = struct.pack(
            "<Q", self._calc_entsize(name, int_sh_type, 64)
        )
    ...
    def _make_sect_header_32(
        ...
        sh_entsize = struct.pack(
            "<I", self._calc_entsize(name, int_sh_type, 32)
        )
    ...
    def _fix_sym_table(self, module, sections_info):
        ...
        syms = obj.Object(
            ...
            vm=module.obj_vm,
            offset=module.symtab,
        )
        for (e, sym) in enumerate(syms):
            if (
                sym.st_value > 0
                and not module.obj_vm.profile.get_symbol_by_address(
                    "kernel", sym.st_value
                )
            ):
                val_map[sym.st_value.v()] = self._find_sec(
                    sections_info, sym.st_value
                )

        for (i, sect) in enumerate(module.get_sections()):
            name_idx_map[str(sect.sect_name)] = (
                i + 1,
                sect.address,
            )  ### account for null segment
        ...
        for sym in syms:
            ...
            if first_name:
                ...
                elif module.obj_vm.profile.get_symbol_by_address(
                    "kernel", sym.st_value
                ):
                    stype = 0  # STT_NOTYPE
    ...
    def _get_module_data(self, module):
        (updated_sections, symtab_idx, load_addr) = self._parse_sections(
            module
        )
        ...
        for (i, (name, address, size, file_off, sect_data)) in enumerate(
            updated_sections
        ):
            section_headers = section_headers + _make_sect_header(
                name, address, size, file_off, strtab_idx, symtab_idx
            )
        ...
        header = _get_header(
            load_addr - hdr_sz, hdr_sz + len(section_data), num_sects
        )

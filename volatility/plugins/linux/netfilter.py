...
class linux_netfilter(linux_common.AbstractLinuxCommand):
    ...
    def calculate(self):
        ...
        if nf_hooks_addr == None:
            debug.error(
                "Unable to analyze NetFilter. It is either disabled or compiled as a module."
            )
        ...
        for outer in range(13):
            ...
            for inner in range(7):
                list_head = obj.Object(
                    "list_head",
                    offset=arr + (inner * list_head_size),
                    vm=self.addr_space,
                )
                ...
                for hook_ops in list_head.list_of_type("nf_hook_ops", "list"):
                    ...
                    yield proto_names[outer], hook_names[
                        inner
                    ], hook_ops.hook.v(), hooked
    ...
    def generator(self, data):
        for outer, inner, hook_addr, hooked in data:
            yield (
                0,
                [str(outer), str(inner), Address(hook_addr), str(hooked)],
            )

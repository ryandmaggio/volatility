...
class linux_netscan(linux_common.AbstractLinuxCommand):
    ...
    def check_pointers(self, i):
        ret = (
            self.addr_space.profile.get_symbol_by_address(
                "kernel", i.sk.sk_backlog_rcv.v()
            )
            != None
        )
    ...
    def check_family(self, i):
        return i.sk.__sk_common.skc_family in (
            socket.AF_INET,
            socket.AF_INET6,
    ...
    def calculate(self):
        ...
        for _, address in scanner.scan(start_offset=kernel_start):
            ...
            i = obj.Object(
                "inet_sock", offset=base_address, vm=self.addr_space
            )
    ...
    def render_text(self, outfd, data):
        for (isock, proto, saddr, sport, daddr, dport, state) in data:
            outfd.write(
                "{6:x} {0:8s} {1:<16}:{2:>5} {3:<16}:{4:>5} {5:<15s}\n".format(
                    proto, saddr, sport, daddr, dport, state, isock.v()
                )
            )

...
class linux_pidhashtable(linux_pslist.linux_pslist):
    ...
    def _task_for_pid(self, upid, pid):
        ...
        if pid_tasks_0 == 0:
            ...
            pnext_addr = (
                upid.obj_offset
                + self.profile.get_obj_offset("upid", "pid_chain")
                + self.profile.get_obj_offset("hlist_node", "next")
            )
            pnext = obj.Object(
                "unsigned long", offset=pnext_addr, vm=self.addr_space
            )
            upid = obj.Object(
                "upid",
                offset=pnext
        ...
        if chained == 0:
            task = obj.Object(
                "task_struct",
                offset=pid_tasks_0
                - self.profile.get_obj_offset("task_struct", "pids"),
                vm=self.addr_space,
            )
            if task.pid > 0:
                yield task
    ...
    def _walk_upid(self, upid):
        ...
        while upid and upid.is_valid() and upid.v() not in seen:
            ...
            if type(upid.pid_chain) == obj.Pointer:
                pid_chain = obj.Object(
                    "hlist_node",
                    offset=upid.pid_chain.obj_offset,
                    vm=self.addr_space,
                )
            else:
    ...
    def _get_pidhash_array(self):
        pidhash_shift = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("pidhash_shift"),
            vm=self.addr_space,
        )
        pidhash_size = 1 << pidhash_shift

        pidhash_addr = self.addr_space.profile.get_symbol("pid_hash")
        pidhash_ptr = obj.Object(
            "Pointer", offset=pidhash_addr, vm=self.addr_space
        )
    ...
    def profile_unsupported(self, func_name):
        debug.error(
            "{0:s}: This profile is currently unsupported by this plugin. Please file a bug report on our issue tracker to have support added.".format(
                func_name
            )
        )
    ...
    def calculate_v2(self):
        ...
        for p in pidhash:
            ...
            nexth = obj.Object(
                "task_struct", offset=nexth - poff, vm=self.addr_space
            )
    ...
    def _walk_idr_node(self, node, height, idx):
        for i in range(self.RADIX_TREE_MAP_SIZE):
            ...
            if height == 1:
                ...
                for child_slot in self._walk_idr_node(
                    slot, height - 1, child_index
                ):
                    yield child_slot
    ...
    def _task_for_radix_pid_node(self, node):
        ...
        if pid_tasks_0 == 0:
            ...
            elif self.addr_space.profile.obj_has_member(
                "task_struct", "pid_links"
            ):
                offset = self.addr_space.profile.get_obj_offset(
                    "task_struct", "pid_links"
                )
            ...
            task = obj.Object(
                "task_struct", offset=pid_tasks_0 - offset, vm=self.addr_space
            )
    ...
    def _do_walk_xarray(self, ff, node, height, index):
        ...
        for i in range(self.XA_CHUNK_SIZE):
            ...
            if (slot.v() & self.XARRAY_TAG_MASK) == self.XARRAY_TAG_INTERNAL:
                slot = obj.Object(
                    "xa_node",
                    offset=slot.v() & ~self.XARRAY_TAG_INTERNAL,
                    vm=self.addr_space,
                )
            ...
            if height == 1:
                ...
                for new_slot in self._do_walk_xarray(
                    ff, slot, height - 1, new_index
                ):
                    yield new_slot

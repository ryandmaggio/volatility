...
class linux_process_stack(linux_process_info.linux_process_info):
    ...
    def __init__(self, config, *args, **kwargs):
        linux_process_info.linux_process_info.__init__(
        ...
        self._config.add_option(
            ...
            short_option='s',
            default=None,
            help='Directory containing files with function symbols',
            type='str',
        )
        self._config.add_option(
            'DUMP-FILE',
            short_option='o',
            default=None,
            help='Dump an annotated stack to this file',
            type='str',
        )
        ...
        if distorm_loaded:
            self.decode_as = (
                ...
                if linux_process_info.address_size == 4
                else distorm3.Decode64Bits
            )
        else:
            debug.error(
                "You really need the distorm3 python module for this plugin to function properly."
            )
    ...
    def load_symbols(self, dir):
        ...
        if os.path.isdir(dir):
            debug.info(
                "Loading function symbols from directory: {}".format(dir)
            )
            symbols = {}
            ...
            debug.warning(
                "Loading function symbols from directory: Not a valid directory: {}".format(
                    dir
                )
            )
        return None
    ...
    def calculate(self):
        ...
        if self._config.DUMP_FILE:
            try:
                ...
                debug.info(
                    "Opened {} for writing".format(self._config.DUMP_FILE)
                )
            except IOError:
                debug.error(
                    "Failed to open %s for writing".format(
                        self._config.DUMP_FILE
                    )
                )
        ...
        for p in linux_process_info.linux_process_info.calculate(self):
            ...
            if p:
                for i, task in enumerate(p.threads):
                    ...
                    debug.info(
                        "Starting analysis of task: pid {}, thread name {}".format(
                            task.pid, task.comm
                        )
                    )
                    debug.info(
                    ...
                    yield self.analyze_stack(
                        p, task, i
                    )  # , self.analyze_registers(p, task, i)
            else:
    ...
    def analyze_stack(self, process_info, task, thread_number):
        ...
        for (low, high) in p.thread_stack_ranges:
            ...
            if low <= p.thread_registers[i].rsp <= high:
                debug.info(
                    "Found the stack at 0x{:016x}-0x{:016x}".format(low, high)
                )
                stack_low = low
                stack_high = high
        ...
        debug.info(
            "Found libc ({}) at range: 0x{:016x}-0x{:016x}".format(
                libc_name, libc_start, libc_end
            )
        )
        debug.info(
            "Program code located at 0x{:016x}-0x{:016x}".format(
        ...
        if not is_thread and entry_point:
            ...
            debug.info(
                "Scanning for return address of __libc_start_main function, starting at program arguments (0x{:016x}) downwards".format(
                    stack_arguments
                )
            )
            ...
            if return_libc_start:
                stats['libc_start'] += 1
                debug.info(
                    "Found the __libc_start_main return address (0x{:016x}) at address 0x{:016x}".format(
                        return_start, return_libc_start
                    )
                )
                ...
                debug.info(
                    "Scanning for return address of main function, starting at __libc_start_main return address (0x{:016x}) downwards".format(
                        return_libc_start
                    )
                )
        ...
        if not main_scan_start:
            if not is_thread:
                ...
                debug.info(
                    "Scanning for return address of main function, starting at program arguments (0x{:016x}) downwards".format(
                        main_scan_start
                    )
                )
            else:
        ...
        found_main = self.find_return_main(
            process_info.proc_as, libc_start, libc_end, main_scan_start
        )
        ...
        if found_main:
            ...
            debug.info(
                "The address of the main function is 0x{:016x}".format(
                    main_address
                )
            )
        ...
        if self.has_frame_pointer(main_address, p.proc_as):
            ...
            frames += self.find_oldschool_frames(
                p, p.proc_as, p.thread_registers[i]
            )
        ...
        if len(frames) > 0:
            ...
            while (
                lastframe.ebp
                and p.is_thread_stack_pointer(lastframe.ebp)
    ...
    def find_scanned_frames(self, p, address, end):
        ...
        frames = []
        debug.info(
            "Scan range (%rsp to end) = (0x{:016x} to 0x{:016x})".format(
                address, end
            )
        )
        count = 0
        while address <= end:
            if p.proc_as.is_valid_address(address) and self.is_return_address(
                read_address(p.proc_as, address, address_size), p
            ):
    ...
    def find_entry_point(self, proc_as, start_code):
        ...
        if not proc_as.is_valid_address(start_code + 0x18):
            ...
            debug.info(
                "We could not find program entry point, skipping _start detection"
            )
            return False
    ...
    def is_return_address(self, address, process_info):
        ...
        if distorm_loaded and process_info.is_code_pointer(address):
            offset = address - size
            instr = distorm3.Decode(
                offset, proc_as.read(offset, size), self.decode_as
            )
    ...
    def find_return_libc_start(self, proc_as, start_stack, return_start):
        ...
        debug.info(
            "Exhausted search for __libc_start_main return address at stack address {:016x}".format(
                address
            )
        )
        return None
    ...
    def find_return_main(self, proc_as, libc_start, libc_end, start_address):
        ...
        def is_return_address(address):
            ...
            single_instr = distorm3.Decode(
                address - size, bytestr, self.decode_as
            )
        ...
        for value in yield_address(proc_as, start_address, reverse=True):
            if libc_start <= value <= libc_end:
                ...
                if not proc_as.is_valid_address(value):
                    ...
                    if retval:
                        debug.info(
                            "Scanned {} libc addresses on the stack before finding the main return address".format(
                                counter
                            )
                        )
                        return addr, retval
        ...
        debug.info(
            "Scanned {} libc addresses on the stack, did not find the main return address".format(
                counter
            )
        )
        debug.info(
    ...
    def is_function_header(self, instructions):
        ...
        return (
            len(instructions) > 1
            and instructions[0][2] == 'PUSH RBP'
            and instructions[1][2] == 'MOV RBP, RSP'
        )
                ...
                if vma.vm_start <= address <= vma.vm_end:
                    ...
                    if base in self.symbols:
                        ...
                        if offset in self.symbols[base]:
                            debug.info(
                                "Instruction was a call to 0x{:016x} = {}@{}".format(
                                    address, self.symbols[base][offset], base
                                )
                            )
                            return self.symbols[base][offset]
                        elif (
                            address in self.symbols[base]
                        ):  # for a function in the main binary, eg 0x40081e
                            debug.info(
                                "Instruction was a call to 0x{:016x} = {}@{}".format(
                                    address, self.symbols[base][address], base
                                )
                            )
    ...
    def find_function_address(self, proc_as, ret_addr):
        ...
        if distorm_loaded:
            ...
            retaddr_assembly = distorm3.Decode(
                ret_addr - 5, proc_as.read(ret_addr - 5, 5), decode_as
            )
            ...
            retaddr_assembly = retaddr_assembly[
                0
            ]  # We're only getting 1 instruction
            ...
            if instr[0] == 'CALL':
                ...
                if instr2[0] == 'JMP':
                    ...
                    debug.info(
                        "Found function address from instruction {} at offset 0x{:016x}".format(
                            instr2, target
                        )
                    )
    ...
    def render_text(self, outfd, data):
        ...
        for (p, reg, frames) in data:
            ...
            print(frames)
            if self.dump_file:
                self.write_annotated_stack(
                    self.dump_file, self.calculate_annotations(frames)
                )
        print(stats)
...
class stack_frame(object):
    ...
    def ebp(self):
        if (
            self.proc_as.is_valid_address(self.ebp_address)
            and self.ebp_address != 0
        ):
    ...
    def __repr__(self):
        ...
        if self.locals_size:
            rep += "Local variables at {:016x} to {:016x}\n".format(
                self.ebp_address, self.locals_end
            )
        ...
        rep += "\tebp at 0x{:016x}: 0x{:016x}\n".format(
            self.ebp_address, self.ebp
        )
        rep += "\teip at 0x{:016x}: 0x{:016x} (Return Address)\n".format(
            self.ret_address, self.ret
        )
        if self.function:

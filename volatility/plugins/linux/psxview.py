...
class linux_psxview(linux_common.AbstractLinuxCommand):
    ...
    def _get_pslist(self):
        return [
            self.addr_space.vtop(x.obj_offset)
    ...
    def _get_pid_hash(self):
        return [
            self.addr_space.vtop(x.obj_offset)
            for x in linux_pidhashtable.linux_pidhashtable(
                self._config
            ).calculate()
        ]
    ...
    def _get_kmem_cache(self):
        return [
            self.addr_space.vtop(x.obj_offset)
            for x in linux_pslist_cache.linux_pslist_cache(
                self._config
            ).calculate()
        ]
    ...
    def _get_thread_leaders(self):
        return [
            self.addr_space.vtop(x.group_leader.v())
            for x in linux_pidhashtable.linux_pidhashtable(
                self._config
            ).calculate()
        ]
    ...
    def _get_psscan(self):
        return [
            x.obj_offset
            for x in linux_psscan.linux_psscan(self._config).calculate()
        ]
        ...
        for source in ps_sources:
            ...
            for offset in tasks:
                ...
                if offset and offset not in seen_offsets:
                    ...
                    yield offset, obj.Object(
                        "task_struct", offset=offset, vm=phys_addr_space
                    ), ps_sources

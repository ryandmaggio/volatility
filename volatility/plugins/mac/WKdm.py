...
class WKdm:
    ...
    def WK_unpack_2bits(self, input_buf, input_start, input_end, output_buf):
        ...
        while input_start < input_end:
            ...
            output_buf[output_next + 1] = (
                temp >> 2
            ) & self.TWO_BITS_PACKING_MASK
            output_buf[output_next + 2] = (
                temp >> 4
            ) & self.TWO_BITS_PACKING_MASK
            output_buf[output_next + 3] = (
                temp >> 6
            ) & self.TWO_BITS_PACKING_MASK
    ...
    ###################################################################
    def WK_unpack_3_tenbits(
        self, input_buf, input_start, input_end, output_buf
    ):
    ...
    ###################################################################
    def WKdm_compress(
        self, src_buf, dest_buf, num_input_words, compression_budget
    ):
        ...
        compression_budget -= (
            self.HEADER_SIZE_IN_WORDS + self.TAGS_AREA_SIZE_IN_WORDS
        ) * self.WORD_SIZE_IN_BYTES
        ...
        while next_input_word < num_input_words:
            ...
            if input_word == dict_word:
                ...
                if input_high_bits == dict_word_high_bits:
                    ...
                    next_qp += 1
                    tempLowBitsArray[next_low_bits] = (
                        input_word & self.LOW_BITS_MASK
                    )
                    next_low_bits += 1
        ...
        boundary_tmp = self.WK_pack_4bits(
            tempQPosArray, endQPosArray, dest_buf, next_full_patt
        )
        ...
        boundary_tmp = self.WK_pack_3_tenbits(
            tempLowBitsArray, endLowBitsArray, dest_buf, boundary_tmp
        )
    ...
    def WKdm_decompress(self, src_buf, dest_buf):
        ...
        try:
            ...
            self.WK_unpack_4bits(
                src_buf, src_buf[0], src_buf[1], tempQPosArray
            )

            self.WK_unpack_3_tenbits(
                src_buf, src_buf[1], src_buf[2], tempLowBitsArray
            )

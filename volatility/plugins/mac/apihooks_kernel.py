...
class mac_apihooks_kernel(common.AbstractMacCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self._config.add_option(
            ...
            help="Check all kext functions in the kext's symbol table for hooking, including kernel symbol table",
            action="store_true",
        )
        self._config.add_option(
            "CHECKKERNEL",
            ...
            default=False,
            cache_invalidator=False,
            help="Check only kernel symbol table functions for hooking",
            action="store_true",
        )
    ...
    def getKextSymbols(
        ...
        if kext_name != None:
            if kext_name in ["kernel", "__kernel__"]:
                kext_addr = self.addr_space.profile.get_symbol(
                    "_g_kernel_kmod_info"
                )
            else:
                ...
                kmodaddr = obj.Object(
                    "Pointer",
                    offset=self.addr_space.profile.get_symbol("_kmod"),
                    vm=self.addr_space,
                )
        ...
        mh = obj.Object(
            mach_header_struct, offset=kext_addr, vm=self.addr_space
        )
        seg_offset = mh.obj_offset + self.addr_space.profile.get_obj_size(
            mach_header_struct
        )

        linkedit_vmaddr = 0  # the first struct nlist is here
        symtab_symoff = (
            0  # specifies the offset in the file to the symbol table
        )
        symtab_stroff = (
            0  # specifies the offset in the file to the string table
        )
        symbol_cnt = 0
        ...
        for i in range(0, mh.ncmds):
            seg = obj.Object(
                segment_command_struct, offset=seg_offset, vm=self.addr_space
            )
            if (
                seg.cmd == 0x19
                and seg.segname
                and str(seg.segname) == "__LINKEDIT"
            ):
                ...
                symtab = obj.Object(
                    'symtab_command', offset=seg_offset, vm=self.addr_space
                )
            ...
            if seg.cmd == LC_SEGMENT:
                ...
                for j in range(0, seg.nsects):
                    ...
                    sect = obj.Object(
                        section_struct,
                        offset=seg_offset
                        + self.addr_space.profile.get_obj_size(
                            segment_command_struct
                        )
                        + (
                            self.addr_space.profile.get_obj_size(
                                section_struct
                    ...
                    if (
                        seg.cmd == 0x19
                        and seg.segname
                        and str(seg.segname) == "__TEXT"
                        and sect_name == "__text"
                    ):
        ...
        for i in range(0, symbol_cnt - 1):
            sym = obj.Object(
                nlist_struct,
                offset=linkedit_vmaddr
                + symbol_offset
                + (i * self.addr_space.profile.get_obj_size(nlist_struct)),
                vm=self.addr_space,
            )
            sym_addr = sym.n_strx + linkedit_vmaddr + string_offset
            sym_name = obj.Object(
                'String', offset=sym_addr, vm=self.addr_space, length=256
            )
    ...
    def findKextWithAddress(self, addr):
        ...
        for kext in kexts:
            if addr >= kext.address and addr <= (
                kext.address + kext.m('size')
            ):
                return kext.name

        return "UNKNOWN"

    def isCallReferenceModified(
        self, model, distorm_mode, func_addr, kernel_syms, kmods
    ):
            ...
            if op.flowControl == 'FC_CALL':
                ...
                if (
                    ...
                    if model == '32bit':
                        ...
                        d = obj.Object(
                            "unsigned int", offset=const, vm=self.addr_space
                        )
                    else:
        ...
        if (
            d
            and self.outside_module(d, kernel_syms, kmods) == True
            and str(ops[n + 1].mnemonic)
            not in ["DB 0xff", "ADD", "XCHG", "OUTS"]
        ):
            modified = True
            ...
            if op_cnt == 2:
                if model == "32bit":
                    if (
                        ...
                        and op.operands[1].type == "Register"
                        and op.operands[0].name == "EBP"
                        and op.operands[1].name == "ESP"
                        and prev_op.mnemonic == "PUSH"
                        and len(prev_op.operands) == 1
                        and prev_op.operands[0].type == "Register"
                        and prev_op.operands[0].name == "EBP"
                    ):
                        pass
                    else:
                        inlined = True
                    ...
                    if (
                        ...
                        and op.operands[0].type == "Register"
                        and op.operands[1].type == "Register"
                        and op.operands[0].name == "RBP"
                        and op.operands[1].name == "RSP"
                        and prev_op.mnemonic == "PUSH"
                        and len(prev_op.operands) == 1
                        and prev_op.operands[0].type == "Register"
                        and prev_op.operands[0].name == "RBP"
                    ):
                        pass
                    elif (
                        prev_op.mnemonic == "PUSH"
                        and len(prev_op.operands) == 1
                        and prev_op.operands[0].type == "Register"
                        and prev_op.operands[0].name == "RBP"
                        and op.mnemonic == "PUSH"
            ...
            if op.flowControl == 'FC_CALL':
                ...
                if (
                    ...
                    if model == '32bit':
                        const = op.operands[0].disp & 0xFFFFFFFF
                        d = obj.Object(
                            "unsigned int", offset=const, vm=addr_space
                        )
                    else:
                        const = op.operands[0].disp
                        d = obj.Object(
                            "unsigned long long", offset=const, vm=addr_space
                        )
                    if self.outside_module(d, kernel_syms, kmods):
                        break
                elif op.operands[0].type == 'Immediate':
                ...
                if op.size > 2:
                    if op.operands[0].type == 'AbsoluteMemoryAddress':
                        ...
                        if model == '32bit':
                            ...
                            const = op.operands[0].disp
                            d = obj.Object(
                                "long long", offset=const, vm=addr_space
                            )
                        if self.outside_module(d, kernel_syms, kmods):
                            break
                    elif op.operands[0].type == 'Immediate':
    ...
    def calculate(self):
        ...
        (kernel_symbol_addresses, kmod) = common.get_kernel_function_addrs(
            self
        )
        ...
        for (i, sysent) in enumerate(sysents):
            ...
            inlined, dst_addr = self.isInlined(
                model, distorm_mode, ent_addr, kernel_symbol_addresses, [kmodk]
            )
            prolog_inlined = self.isPrologInlined(
                model, distorm_mode, ent_addr
            )
            if hooked == True or inlined == True or prolog_inlined == True:
                ...
                if ent_name != "_nosys" and ent_name in dict_syscall_funcs:
                    ...
                    yield (
                        "DuplicateSyscall -> {0}".format(ent_name),
                        i,
                        ent_addr,
                        True,
                        False,
    ...
    def generator(self, data):
        for (
            ...
            if hooked == False:
                sym_name = self.profile.get_symbol_by_address_type(
                    "kernel", call_addr, "N_FUN"
                )

...
class mac64_bash_hash_table(bash_funcs):
    ...
    def is_valid(self):
        if (
            ...
            or not self.obj_vm.is_valid_address(self.bucket_array)
            or not self.nbuckets == 64
            or not self.nentries >= 0
        ):
            return False
    ...
    def __iter__(self):
        if self.is_valid():
            ...
            for bucket_ptr in bucket_array:
                ...
                while (
                    bucket.is_valid()
                    and bucket.times_found > 0
                    and bucket.data.is_valid()
                    and bucket.key != ""
                ):
                    yield bucket

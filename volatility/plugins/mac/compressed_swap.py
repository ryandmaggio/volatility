...
class mac_compressed_swap(common.AbstractMacCommand):
    ...
    def calculate(self):
        ...
        com_obj_addr = self.addr_space.profile.get_symbol(
            "_compressor_object_store"
        )

        if not com_obj_addr:
            debug.error(
                "The given memory sample does not utilize compressed swap."
            )

        # from osfmk/vm/vm_object.h.  compressor_object is the high level VM object.
        compressor_object = obj.Object(
            "vm_object", offset=com_obj_addr, vm=self.addr_space
        )
        ...
        c_segment_count = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("_c_segment_count"),
            vm=self.addr_space,
        )

        c_segments_ptr = obj.Object(
            "Pointer",
            offset=self.addr_space.profile.get_symbol("_c_segments"),
            vm=self.addr_space,
        )

        c_segments = obj.Object(
            "Array",
            targetType="c_segu",
            count=c_segment_count,
            offset=c_segments_ptr,
            vm=self.addr_space,
        )

        c_segments_available = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("_c_segments_available"),
            vm=self.addr_space,
        )

        c_segments_busy = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("_c_segments_busy"),
            vm=self.addr_space,
        )

        c_segment_compressed_bytes = obj.Object(
            "long long",
            offset=self.addr_space.profile.get_symbol(
                "_c_segment_compressed_bytes"
            ),
            vm=self.addr_space,
        )
        ...
        compressor_bytes_used = obj.Object(
            "long long",
            offset=self.addr_space.profile.get_symbol(
                "_compressor_bytes_used"
            ),
            vm=self.addr_space,
        )
        ...
        vm_page_active_count = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("_vm_page_active_count"),
            vm=self.addr_space,
        )
        vm_page_inactive_count = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol(
        ...
        vm_page_free_count = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol("_vm_page_free_count"),
            vm=self.addr_space,
        )
        vm_page_speculative_count = obj.Object(
            "unsigned int",
            offset=self.addr_space.profile.get_symbol(
        ...
        yield (
            "Available uncompressed memory",
            available_uncompressed,
            "pages",
        )

        available_memory = (
            available_uncompressed + compressor_object.resident_page_count
        )
        ...
        for i in range(c_segment_count):
            ...
            if (
                c_segments[i].c_seg.c_bytes_used < 1
                or c_segments[i].c_seg.c_bytes_used > self.C_SEG_ALLOCSIZE
            ):
                yield (
                    "Segment " + str(i) + " size is invalid",
                    "SKIPPING",
                    "",
                )
                continue
            ...
            yield (
                "Segment " + str(i),
                c_segments[i].c_seg.c_bytes_used,
                "bytes used",
            )
            yield (
                "Segment " + str(i),
                c_segments[i].c_seg.c_bytes_unused,
                "bytes unused",
            )
            ...
            while (
                j1 < self.C_SEG_SLOT_ARRAYS
                and j1 * self.C_SEG_SLOT_ARRAY_SIZE + j2 < c_nextslot
            ):
                ...
                if cslot_array.is_valid():
                    cslots = obj.Object(
                        ...
                        offset=cslot_array,
                        targetType="c_slot",
                        count=self.C_SEG_SLOT_ARRAY_SIZE,
                        vm=self.addr_space,
                    )
                    while (
                        j2 < self.C_SEG_SLOT_ARRAY_SIZE
                        and j1 * self.C_SEG_SLOT_ARRAY_SIZE + j2 < c_nextslot
                    ):
                        cslot = cslots[j2]
                        ...
                        if csize > 0:
                            yield (
                                ...
                                + str(j2)
                                + " offset",
                                str(cslot.c_offset * 4),
                                "bytes",
                            )
                            yield (
                                "  Slot " + str(j1) + ", " + str(j2) + " size",
                                str(csize * 4),
                                "bytes " + status,
                            )

                            cslot_data = obj.Object(
                                "Array",
                                offset=c_segments[i].c_seg.c_store.c_buffer
                                + cslot.c_offset * 4,
                                targetType="int",
                                count=csize,
                                vm=self.addr_space,
                            ...
                            if compressed:
                                ...
                                decompressed = self.wkdm.WKdm_decompress(
                                    cslot_data, self.dest
                                )
                                if decompressed > 0:

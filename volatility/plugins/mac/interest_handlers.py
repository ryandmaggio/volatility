...
class mac_interest_handlers(common.AbstractMacCommand):
    ...
    def walk_reg_entry(self, reg_addr):
        ...
        for ent in ents:
            ...
            if key == "IODeviceMemory":
                current_name = str(
                    ent.value.dereference_as(self._struct_or_class("OSString"))
                )
                device_mem = True
    ...
    def walk_child_links(self, addr):
        val = obj.Object(
            self._struct_or_class("OSArray"), offset=addr, vm=self.addr_space
        )
    ...
    def calculate(self):
        ...
        regroot_addr = common.get_cpp_sym(
            "gRegistryRoot", self.addr_space.profile
        )

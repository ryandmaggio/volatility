...
class mac_network_conns(common.AbstractMacCommand):
    ...
    def _walk_pcb_hash(self, proto_pcbinfo):
        pcb_hash = obj.Object(
            "Array",
            offset=proto_pcbinfo.hashbase,
            vm=self.addr_space,
            targetType="Pointer",
            count=proto_pcbinfo.hashmask + 1,
        )
    ...
    def _walk_pcb_entries(self, inpcbinfo_addr):
        ...
        inpcbinfo = obj.Object(
            "inpcbinfo", offset=inpcbinfo_addr, vm=self.addr_space
        )
    ...
    def calculate(self):
        ...
        for (proto_str, info_addr) in info_addrs:
            for (pcbinfo, lip, lport, rip, rport) in self._walk_pcb_entries(
                info_addr
            ):
                if proto_str == "TCP":
    ...
    def render_text(self, outfd, data):
        ...
        for (proto, pcb, lip, lport, rip, rport, state) in data:
            self.table_row(
                outfd, pcb.obj_offset, proto, lip, lport, rip, rport, state
            )

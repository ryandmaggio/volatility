...
class mac_psxview(common.AbstractMacCommand):
    ...
    def _get_pid_hash_table(self):
        return [
            p.v()
            for p in pid_hash_table.mac_pid_hash_table(
                self._config
            ).calculate()
        ]
    ...
    def _get_pgrp_hash_table(self):
        return [
            p.v()
            for p in pgrp_hash_table.mac_pgrp_hash_table(
                self._config
            ).calculate()
        ]
    ...
    def _get_session_hash_table(self):
        return [
            s.s_leader.v()
            for s in session_hash_table.mac_list_sessions(
                self._config
            ).calculate()
            if s.s_leader.is_valid()
        ]
            ...
            for offset in tasks:
                if offset not in seen_offsets:
                    ...
                    yield offset, obj.Object(
                        "proc", offset=offset, vm=self.addr_space
                    ), ps_sources

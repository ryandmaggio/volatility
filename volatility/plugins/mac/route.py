...
class mac_route(common.AbstractMacCommand):
    ...
    def _get_table(self, tbl):
        rnh = (
            tbl  # obj.Object("radix_node", offset=tbl.v(), vm=self.addr_space)
        )
        rn = rnh.rnh_treetop
        ...
        while rn.is_valid():
            ...
            while (
                rn.is_valid()
                and rn.rn_parent.rn_u.rn_node.rn_R == rn
                and rn.rn_flags & 2 == 0
            ):
                if rn.v() in seen:
                    break
    ...
    def calculate(self):
        ...
        ipv4table = obj.Object(
            "radix_node_head", offset=ents[2], vm=self.addr_space
        )

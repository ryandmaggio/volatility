...
class mac_vfsevents(common.AbstractMacCommand):
    ...
    def calculate(self):
        ...
        if not self.addr_space.profile.obj_has_member(
            "fs_event_watcher", "proc_name"
        ):
            debug.error(
                "This plugin only supports OS X >= 10.8.2. Please file a bug if you are running against a version matching this criteria."
            )
        ...
        for watcher_addr in arr:
            ...
            events = ""
            event_arr = obj.Object(
                theType="Array",
                targetType="unsigned char",
                offset=watcher.event_list.v(),
                count=13,
                vm=self.addr_space,
            )

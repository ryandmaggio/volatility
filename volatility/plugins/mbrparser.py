...
class PARTITION_ENTRY(obj.CType):
    ...
    def get_type(self):
        return PartitionTypes.get(
            self.get_value(self.PartitionType), "Invalid"
        )
    ...
    def StartingCylinder(self):
        return (
            self.StartingCHS[1] - self.StartingSector()
    ...
    def EndingCylinder(self):
        return (self.EndingCHS[1] - self.EndingSector()) * 4 + self.EndingCHS[
            2
        ]
...
class MBRParser(commands.Command):
    ...
    def calculate(self):
        ...
        if self._config.MAXDISTANCE != None and not self._config.DISK:
            debug.error(
                "Must supply the path for the extracted MBR/Disk when using MAXDISTANCE"
            )
        ...
        if self._config.OFFSET:
            PARTITION_TABLE = obj.Object(
                'PARTITION_TABLE', vm=address_space, offset=self._config.OFFSET
            )
            boot_code = address_space.read(
    ...
    def _get_instructions(self, boot_code):
        if self._config.HEX:
            return "".join(
                [
                    "{2}".format(o, h, ''.join(c))
                    for o, h, c in self.Hexdump(boot_code, 0)
                ]
            )
        iterable = distorm3.DecodeGenerator(
            0, boot_code, distorm3.Decode16Bits
        )
        ret = ""
    ...
    def generator(self, data):
        ...
        for offset, PARTITION_TABLE, boot_code in data:
            ...
            have_bootable = (
                entry1.is_bootable_and_used()
                or entry2.is_bootable_and_used()
                or entry3.is_bootable_and_used()
            ...
            if self.disk_mbr:
                ...
                if (
                    self._config.MAXDISTANCE != None
                    and distance > self._config.MAXDISTANCE
                ):
                    continue
            ...
            yield (
                ...
                [
                    ...
                    "{0:#x} ({1})".format(
                        entry1.get_value(entry1.PartitionType),
                        entry1.get_type(),
                    ),
                    Hex(entry1.StartingLBA),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry1.StartingCylinder(),
                        entry1.StartingCHS[0],
                        entry1.StartingSector(),
                    ),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry1.EndingCylinder(),
                        entry1.EndingCHS[0],
                        entry1.EndingSector(),
                    ),
                    ...
                    "{0:#x} ({1})".format(
                        entry2.get_value(entry2.PartitionType),
                        entry2.get_type(),
                    ),
                    Hex(entry2.StartingLBA),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry2.StartingCylinder(),
                        entry2.StartingCHS[0],
                        entry2.StartingSector(),
                    ),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry2.EndingCylinder(),
                        entry2.EndingCHS[0],
                        entry2.EndingSector(),
                    ),
                    ...
                    "{0:#x} ({1})".format(
                        entry3.get_value(entry3.PartitionType),
                        entry3.get_type(),
                    ),
                    Hex(entry3.StartingLBA),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry3.StartingCylinder(),
                        entry3.StartingCHS[0],
                        entry3.StartingSector(),
                    ),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry3.EndingCylinder(),
                        entry3.EndingCHS[0],
                        entry3.EndingSector(),
                    ),
                    ...
                    "{0:#x} ({1})".format(
                        entry4.get_value(entry4.PartitionType),
                        entry4.get_type(),
                    ),
                    Hex(entry4.StartingLBA),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry4.StartingCylinder(),
                        entry4.StartingCHS[0],
                        entry4.StartingSector(),
                    ),
                    "Cylinder: {0} Head: {1} Sector: {2}".format(
                        entry4.EndingCylinder(),
                        entry4.EndingCHS[0],
                        entry4.EndingSector(),
                    ),
        ...
        for offset, PARTITION_TABLE, boot_code in data:
            ...
            if not self._config.HEX:
                ...
                hexstuff = "\n" + "\n".join(
                    [
                        "{0:010x}  {1:<48}  {2}".format(o, h, ''.join(c))
                        for o, h, c in self.Hexdump(boot_code, start)
                    ]
                )
            ...
            outfd.write("{0}\n".format(border))
            outfd.write(
                "Potential MBR at physical offset: {0:#x}\n".format(offset)
            )
            outfd.write(
                "Disk Signature: {0:02x}-{1:02x}-{2:02x}-{3:02x}\n".format(
                    PARTITION_TABLE.DiskSignature[0],
                    PARTITION_TABLE.DiskSignature[1],
                    PARTITION_TABLE.DiskSignature[2],
                    PARTITION_TABLE.DiskSignature[3],

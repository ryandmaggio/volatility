...
class _TCP_LISTENER(obj.CType):
    ...
    def is_valid(self):
        return obj.CType.is_valid(self) and self.AddressFamily in (
            AF_INET,
            AF_INET6,
        )
...
class _TCP_ENDPOINT(_TCP_LISTENER):
    ...
    def LocalAddress(self):
        inaddr = (
            self.AddrInfo.dereference().Local.pData.dereference().dereference()
        )
    ...
    def is_valid(self):
        ...
        if self.State.v() not in tcpip_vtypes.TCP_STATE_ENUM or (
            not self.LocalAddress
            and (
                not self.Owner
                or self.Owner.UniqueProcessId == 0
                or self.Owner.UniqueProcessId > 65535
            )
        ):
            return False
...
class Netscan(common.AbstractScanCommand):
    ...
    def is_valid_profile(profile):
        return (
            ...
            and profile.metadata.get('major', 0) == 6
        )
    ...
    def calculate(self):
        ...
        for objct in self.scan_results(addr_space):
            ...
            if isinstance(objct, _UDP_ENDPOINT):
                ...
                yield objct, proto, objct.LocalAddress, objct.LocalPort, objct.RemoteAddress, objct.RemotePort, objct.State

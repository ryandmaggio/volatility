...
linux_overlay = {
    ...
    'vm_area_struct': [
        None,
        {
            'vm_flags': [
                None,
                ['LinuxPermissionFlags', {'bitmap': {'r': 0, 'w': 1, 'x': 2}}],
            ],
...
def LinuxProfileFactory(profpkg):
    ...
    for f in profpkg.filelist:
        if f.filename.lower().endswith('.dwarf'):
            ...
            arch, memmodel, sysmap = parse_system_map(
                profpkg.read(f.filename), "kernel"
            )
    ...
    class AbstractLinuxProfile(obj.Profile):
        ...
        native_mapping = {
            '32bit': native_types.x86_native_types,
        ...
        def _merge_anonymous_members(self, vtypesvar):
            ...
            try:
                for candidate in vtypesvar:
                    ...
                    while not done:
                        if any(
                            member.startswith('__unnamed_')
                            for member in vtypesvar[candidate][members_index]
                        ):
                            for member in list(
                                vtypesvar[candidate][members_index].keys()
                            ):
                                if member.startswith('__unnamed_'):
                                    member_type = vtypesvar[candidate][
                                        members_index
                                    ][member][types_index][0]
                                    location = vtypesvar[candidate][
                                        members_index
                                    ][member][offset_index]
                                    vtypesvar[candidate][members_index].update(
                                        vtypesvar[member_type][members_index]
                                    )
                                    for name in list(
                                        vtypesvar[member_type][
                                            members_index
                                        ].keys()
                                    ):
                                        vtypesvar[candidate][members_index][
                                            name
                                        ][offset_index] += location
                                    del vtypesvar[candidate][members_index][
                                        member
                                    ]
                ...
                raise exceptions.VolatilityException(
                    "Inconsistent linux profile - unable to look up " + str(e)
                )
        ...
        def load_vtypes(self):
            ...
            debug.debug(
                "{2}: Found dwarf file {0} with {1} symbols".format(
                    f.filename, len(list(vtypesvar.keys())), profilename
                )
            )
        ...
        def load_sysmap(self):
            ...
            debug.debug(
                "{2}: Found system file {0} with {1} symbols".format(
        ...
        def get_all_symbol_names(self, module="kernel"):
            ...
            if module in symtable:
                ...
                debug.error(
                    "get_all_symbol_names called on non-existent module"
                )
    ...
    for path, _, files in os.walk(path):
        for fn in files:
            if zipfile.is_zipfile(os.path.join(path, fn)):
                new_classes.append(
                    LinuxProfileFactory(
                        zipfile.ZipFile(os.path.join(path, fn))
                    )
                )
...
class hlist_node(obj.CType):
    ...
    def list_of_type(
        self, obj_type, member, offset=-1, forward=True, head_sentinel=True
    ):
...
class inet_sock(obj.CType):
    ...
    def dst_port(self):
        if (
            hasattr(self, "sk")
            and hasattr(self.sk, "__sk_common")
            and hasattr(self.sk.__sk_common, "skc_portpair")
        ):
            return socket.htons(
                self.sk.__sk_common.skc_portpair & 0xFFFF
            ...
            hasattr(self, "sk")
            and hasattr(self.sk, "__sk_common")
            and hasattr(self.sk.__sk_common, "skc_dport")
        ):
            return socket.htons(
                self.sk.__sk_common.skc_dport
            )  # pylint: disable-msg=W0212
        else:
            return None
    ...
    def dst_addr(self):
        if self.sk.__sk_common.skc_family == socket.AF_INET:
            ...
            if hasattr(self.pinet6, "daddr"):
                ...
                return self.sk.__sk_common.skc_v6_daddr.cast(
                    "Ipv6Address"
                )  # pylint: disable-msg=W0212
...
class module_struct(obj.CType):
    ...
    def get_sections(self):
        ...
        attrs = obj.Object(
            theType='Array',
            offset=self.sect_attrs.attrs.obj_offset,
            vm=self.obj_vm,
            targetType='module_sect_attr',
            count=num_sects,
        )
    ...
    def get_param_val(self, param, _over=0):
        ...
        if getfn == 0:
            ...
            val = str(
                param.str.dereference_as("String", length=param.str.maxlen)
            )
    ...
    def get_params(self):
        ...
        params = ""
        param_array = obj.Object(
            theType='Array',
            offset=self.kp,
            vm=self.obj_vm,
            targetType='kernel_param',
            count=self.num_kp,
        )

        for param in param_array:
            val = self.get_param_val(param)
            params = params + "{0}={1} ".format(
                param.name.dereference_as("String", length=255), val
            )
    ...
    def is_valid(self):
        ...
        if (
            self.state.v() in [0, 1, 2]
            and self.core_size >= 1
            and self.core_size <= 1000000
            and self.core_text_size >= 1
            and self.core_text_size <= 1000000
        ):
...
class vm_area_struct(obj.CType):
    def is_valid(self):
        ...
        if (
            (start > end)
            or (end - start > 100000000000)
            or (start > 0xFF00000000000000)
            or (end > 0xFF00000000000000)
            or (pgoff > 100000000000)
        ):
    ...
    def vm_name(self, task):
        if self.vm_file:
            ...
            self.vm_start <= task.mm.start_stack
            and self.vm_end >= task.mm.start_stack
        ):
            fname = "[stack]"
        elif (
            hasattr(self.vm_mm.context, "vdso")
...
class task_struct(obj.CType):
    ...
    def bash_hash_entries(self):
        nbuckets_offset = self.obj_vm.profile.get_obj_offset(
            "_bash_hash_table", "nbuckets"
        )
        ...
        for off in self.search_process_memory(
            ...
            htable = obj.Object(
                "_bash_hash_table", offset=off - nbuckets_offset, vm=proc_as
            )
    ...
    def plt_hook_info(self):
        ...
        for k, v in elfs.items():
            ...
            for reloc in elf.relocations():
                ...
                if elf_arch == 32:
                    addr = obj.Object(
                        "unsigned int", offset=offset, vm=elf.obj_vm
                    )
                else:
                    addr = obj.Object(
                        "unsigned long long", offset=offset, vm=elf.obj_vm
                    )
    ...
    def _is_api_hooked(self, sym_addr, proc_as):
        ...
        if (
            self.obj_vm.profile.metadata.get('memory_model', '32bit')
            == '32bit'
        ):
    ...
    def apihook_info(self):
        for (
            ...
            if (
                not hook_vma
                or not hook_func_vma
                or hook_vma.vm_start != hook_func_vma.vm_start
            ):
    ...
    def bash_history_entries(self):
        ...
        for (idx, ptr_string) in enumerate(
            self.search_process_memory(bang_addrs, heap_only=True)
        ):
            # Check if we found a valid history entry object
            hist = obj.Object(
                "_hist_entry", offset=ptr_string - ts_offset, vm=proc_as
            )
    ...
    def _dynamic_env(self, proc_as, pack_format, addr_sz):
        ...
        for vma in self.get_proc_maps():
            ...
            if fname.find("ld") == -1 and (
                not fname.endswith(("/bin/bash", "/bin/dash", "/bin/sh"))
            ):
                continue
    ...
    def _shell_variables(self, proc_as, pack_format, addr_sz):
        ...
        for vma in self.get_proc_maps():
            ...
            for off in range(0, vma_len - addr_sz, 4):
                ...
                htable = obj.Object(
                    "_bash_hash_table", offset=ptr2, vm=proc_as
                )

                for ent in htable:
                    key = str(ent.key.dereference())
                    val = str(
                        ent.data.dereference_as("_envdata").value.dereference()
                    )
    ...
    def netstat(self):
        ...
        for (filp, fdnum) in self.lsof():
            if filp.f_op == sfop or filp.dentry.d_op == dfop:
                ...
                inet_sock = obj.Object(
                    "inet_sock", offset=skt.sk, vm=self.obj_vm
                )

                if inet_sock.protocol in (
                    "TCP",
                    "UDP",
                    "IP",
                    "HOPOPT",
                ):  # hopopt is where unix sockets end up on linux
                    state = (
                        inet_sock.state if inet_sock.protocol == "TCP" else ""
                    )
                    family = (
                        inet_sock.sk.__sk_common.skc_family
                    )  # pylint: disable-msg=W0212

                    if family == 1:  # AF_UNIX
                        unix_sock = obj.Object(
                            "unix_sock",
                        ...
                        if unix_sock.addr:
                            name_obj = obj.Object(
                                "sockaddr_un",
                                offset=unix_sock.addr.name.obj_offset,
                                vm=self.obj_vm,
                            )
    ...
    def threads(self):
        thread_offset = self.obj_vm.profile.get_obj_offset(
            "task_struct", "thread_group"
        )
        threads = [self]
        x = obj.Object(
            'task_struct',
            self.thread_group.next.v() - thread_offset,
            self.obj_vm,
        )
        while (
            x not in threads
            and x.is_valid()
            and x.thread_group.is_valid()
            and x.thread_group.next.is_valid()
        ):
            threads.append(x)
            x = obj.Object(
                'task_struct',
                x.thread_group.next.v() - thread_offset,
                self.obj_vm,
            )
        return threads
    ...
    def get_proc_maps(self):
        ...
        for vma in linux_common.walk_internal_list(
            "vm_area_struct", "vm_next", self.mm.mmap
        ):
            val = vma.v()
            if val in seen:
                break
    ...
    def search_process_memory(self, s, heap_only=False):
        ...
        for vma in self.get_proc_maps():
            if heap_only:
                if not (
                    vma.vm_start <= self.mm.brk
                    and vma.vm_end >= self.mm.start_brk
                ):
                    continue
    ...
    def get_task_start_time(self):
        ...
        if (
            type(start_time) == volatility.obj.NativeType
        ...
        if boot_time != -1:
            ...
            bufferas = addrspace.BufferAddressSpace(
                self.obj_vm.get_config(), data=data
            )
            dt = obj.Object(
                "UnixTimeStamp", offset=0, vm=bufferas, is_utc=True
            )
        else:
            dt = None
...
class timespec(obj.CType):
    def as_timestamp(self):
        ...
        time_buf = addrspace.BufferAddressSpace(
            self.obj_vm.get_config(), data=time_val
        )
        time_obj = obj.Object(
            "UnixTimeStamp", offset=0, vm=time_buf, is_utc=True
        )
...
class VolatilityDTB(obj.VolatilityMagic):
    ...
    def generate_suggestions(self):
        ...
        if config.PHYSICAL_SHIFT and not config.VIRTUAL_SHIFT:
            debug.error(
                "You must specifiy both the virtual and physical shift."
            )
        ...
        try:
            sched_class_offset = profile.get_obj_offset(
                "task_struct", "sched_class"
            )
            idle_class_addr = (
                tbl["idle_sched_class"][0][0] + virtual_shift_address
            )
        except KeyError:
        ...
        if physical_shift_address != 0 and virtual_shift_address != 0:
            good_dtb = (
                dtb_sym_addr - shifts[0] - virtual_shift_address
            ) + physical_shift_address
        ...
        if good_dtb == -1:
            ...
            for swapper_offset in scanner.scan(self.obj_vm):
                ...
                tmp_physical_shift = swapper_address - (
                    init_task_addr - shifts[0]
                )
                ...
                if sched_class_offset != -1:
                    sched_class_val = pas.read(
                        swapper_address + sched_class_offset, read_sz
                    )
    ...
    def generate_suggestions(self):
        ...
        for shift in shifts:
            check = (
                init_task_addr
                - shift
                + self.obj_vm.profile.physical_shift
                - self.obj_vm.profile.virtual_shift
            )
            if phys == check:
                ret = True
                break
    ...
    def to_paddr(self):
        ...
        if mem_map_addr:
            ...
            if mem_map_ptr_addr:
                # (later) ASLR kernels
                mem_map_ptr = obj.Object(
                    "unsigned long", offset=mem_map_ptr_addr, vm=self.obj_vm
                )
            else:
            ...
            debug.error(
                "phys_addr_of_page: Unable to determine physical address of page. NUMA is not supported at this time.\n"
            )
...
class vfsmount(obj.CType):
    def is_valid(self):
        return (
            self.mnt_sb.is_valid()
            and self.mnt_root.is_valid()
            and self.mnt_parent.is_valid()
        )
    ...
    def _get_real_mnt(self):
        ...
        mnt = obj.Object(
            "mount", offset=self.obj_offset - offset, vm=self.obj_vm
        )
        return mnt

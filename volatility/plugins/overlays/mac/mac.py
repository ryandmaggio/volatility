...
class BashEnvYaraScanner(malfind.BaseYaraScanner):
    ...
    def __init__(self, task=None, **kwargs):
        ...
        """
        self.task = task
        malfind.BaseYaraScanner.__init__(
            self, address_space=task.get_process_address_space(), **kwargs
        )
    ...
    def _get_dtb_m_lion(self):
        ...
        if config.SHIFT:
            ...
            for catfish_offset in scanner.scan(self.obj_vm):
                tmp_shift_address = catfish_offset - (
                    tbl["_lowGlo"][0][0] % 0xFFFFFF80
                )
        ...
        boot_pml4_dtb = amd64.AMD64PagedMemory(
            self.obj_vm, config, dtb=bootpml4
        )

        idlepml4_addr = (tbl['_IdlePML4'][0][0]) + shift_address
        idlepml4_ptr = obj.Object(
            "unsigned int", offset=idlepml4_addr, vm=boot_pml4_dtb
        )
...
class ifnet(obj.CType):
    def sockaddr_dl(self):
        if hasattr(self, "if_lladdr"):
            ret = obj.Object(
                "sockaddr_dl",
                offset=self.if_lladdr.ifa_addr.v(),
                vm=self.obj_vm,
            )
        else:
            ret = obj.Object(
                "sockaddr_dl",
                offset=self.if_addrhead.tqh_first.ifa_addr.v(),
                vm=self.obj_vm,
            )
...
class vnode(obj.CType):
    ...
    def _do_calc_path(self, ret, vnodeobj, vname, vnode_offsets):
        ...
        if vnodeobj.v_flag.v() & 0x000001 != 0 and vnodeobj.v_mount.v() != 0:
            ...
            self._do_calc_path(
                ret, vnodeobj.v_parent, vnodeobj.v_parent.v_name, vnode_offsets
            )

    def full_path(self):
        if (
            self.v_flag.v() & 0x000001 != 0
            and self.v_mount.v() != 0
            and self.v_mount.mnt_flag.v() & 0x00004000 != 0
        ):
            ret = "/"
        else:
            elements = []
    ...
    def _get_next_page(self, memq):
        ...
        if p == 0 or p == None:
            ...
            self.obj_vm.profile.metadata.get('memory_model', 0) == "64bit"
            and p.size() == 4
        ):

            if p & 0x80000000 != 0:
                vm_pages_ptr = self.obj_vm.profile.get_symbol("_vm_pages")
                vm_pages_addr = obj.Object(
                    "unsigned long long", offset=vm_pages_ptr, vm=self.obj_vm
                )
                ret_addr = vm_pages_addr + (
                    (p & ~0x80000000)
                    * self.obj_vm.profile.get_obj_size("vm_page")
                )
            else:
...
class kauth_scope(obj.CType):
    ...
    def listeners(self):
        ls_array = obj.Object(
            theType="Array",
            targetType="kauth_local_listener",
            offset=self.m("ks_listeners").obj_offset,
            vm=self.obj_vm,
            count=16,
        )
        for ls in ls_array:
...
class thread(obj.CType):
    def start_time(self):
        ...
        boot_time = obj.Object(
            "unsigned long long", offset=baddr, vm=self.obj_vm
        )
...
class proc(obj.CType):
        ...
        for map in self.get_proc_maps():
            ...
            while chunk_off < end:
                ...
                while off < range_end:
                    ...
                    dr = data[
                        off + nbuckets_offset : off + nbuckets_offset + 4
                    ]
                    ...
                    htable = obj.Object(
                        htable_type, offset=read_off, vm=proc_as
                    )

                    if htable.is_valid():
                        bucket_array = obj.Object(
                            theType="Array",
                            targetType=addr_type,
                            offset=htable.bucket_array,
                            vm=htable.nbuckets.obj_vm,
                            count=64,
                        )
                        seen = set()
                        ...
                        for bucket_ptr in bucket_array:
                            ...
                            while bucket != None and bucket.times_found > 0:
                                ...
                                if (
                                    bucket.key != None
                                    and bucket.data != None
                                    and pdata.is_valid()
                                    and (0 <= pdata.flags <= 2)
                                ):
                                    if (
                                        len(str(bucket.key)) > 0
                                        or len(str(bucket.data.path)) > 0
                                    ) and (0 < bucket.times_found <= 1024):
                                        yield bucket
    ...
    def bash_history_entries(self):
        ...
        for (idx, ptr_string) in enumerate(
            self.search_process_memory_rw_nofile(bang_addrs)
        ):
            # Check if we found a valid history entry object
            hist = obj.Object(
                hist_struct, offset=ptr_string - ts_offset, vm=proc_as
            )
    ...
    def _get_libc_range(self, proc_as):
        ...
        if libc_map:
            mh = obj.Object(
                "macho_header", offset=libc_map.imageLoadAddress, vm=proc_as
            )

            for seg in mh.segments():
                if str(seg.segname) == "__DATA":
                    mapping = [
                        [seg.vmaddr, seg.vmaddr + seg.vmsize, seg.vmsize]
                    ]
    ...
    def _get_env_mappings(self, proc_as):
        ...
        if not mappings:
            ...
            for mapping in self.get_proc_maps():
                if (
                    str(mapping.get_perms()) != "rw-"
                    or mapping.get_path() == ""
                ):
                    continue

                mappings.append(
                    [mapping.start, mapping.end, mapping.end - mapping.start]
                )
            ...
            while chunk_offset < end:
                ...
                while off < 4096 - 4:
                    ...
                    if addr:
                        ...
                        if (
                            not firstaddrstr
                            or len(firstaddrstr) != self.pack_size
                        ):
                            continue
                        firstaddr = struct.unpack(self.pack_fmt, firstaddrstr)[
                            0
                        ]
        ...
        for var in envars:
            ...
            if good_varstr:
                ...
                if (
                    len(key) > 0
                    and len(val) > 0
                    and self._valid_string(key)
                    and self._valid_string(val)
                ):
    ...
    def _shell_variables(self, proc_as, pack_format, addr_sz, htable_type):
        ...
        for hit, off in scanner.scan():
            ...
            for ent in htable:
                ...
                if val_addr.is_valid() and self._valid_string(
                    val_addr.dereference()
                ):
    ...
    def psenv(self):
        ...
        if str(self.p_comm) == "bash":
            for key, val in self._shell_variables(
                proc_as, pack_format, addr_sz, htable_type
            ):
                yield key, val
    ...
    def netstat(self):
        for (filp, _, _) in self.lsof():
            if (
                filp.f_fglob.is_valid()
                and filp.f_fglob.fg_type == 'DTYPE_SOCKET'
            ):
                ...
                if family == 1:
                    ...
                    if vals:
                        ...
                        yield (
                            family,
                            (socket, proto, lip, lport, rip, rport, state),
                        )
    ...
    def p_gid(self):
        ...
        if hasattr(cred, "cr_posix"):
            try:
                ...
                ret = obj.Object(
                    "unsigned int",
                    offset=cred.cr_posix.cr_groups.obj_offset,
                    vm=self.obj_vm,
                )
        else:
    ...
    def get_process_address_space(self):
        ...
        if map_val == "TASK_MAP_32BIT" and is_64bit_cap:
            ...
            try:
                proc_as = amd64.AMD64PagedMemory(
                    self.obj_vm.base,
            ...
            proc_as = intel.IA32PagedMemoryPae(
                self.obj_vm.base,
                self.obj_vm.get_config(),
                dtb=cr3,
                skip_as_check=True,
            ...
            map_val == "TASK_MAP_64BIT_SHARED"
            and self.obj_vm.profile.metadata.get('memory_model', '32bit')
            == "32bit"
        ):
        ...
        if wanted_vnode:
            for map in self.get_proc_maps():
                ...
                if (
                    vnode
                    and vnode != "sub_map"
                    and vnode.v() == wanted_vnode
                    and map.get_perms() == "r-x"
                ):
    ...
    def get_dyld_maps(self):
        ...
        infos = obj.Object(
            dtype, offset=self.task.all_image_info_addr, vm=proc_as
        )
        if not infos:
            return
    ...
    def get_arguments(self):
        ...
        if (
            not proc_as.is_valid_address(argsstart)
            or self.p_argslen == 0
            or self.p_argc == 0
        ):
            return ""
        ...
        while argc > 0:
            arg = obj.Object(
                "String", offset=argsstart, vm=proc_as, length=256
            )
            ...
            if len(args) == 0:
                while (
                    proc_as.read(argsstart, 1) == "\x00"
                    and argsstart < self.user_stack
                ):
                    argsstart += 1
                args.append(arg)
            else:
...
class rtentry(obj.CType):
    def is_valid(self):
        return (
            str(self.source_ip) != ""
            and str(self.dest_ip) != ""
            and (0 <= int(self.sent) < 50000000000)
            and (0 <= int(self.rx) < 50000000000)
        )
    ...
    def name(self):
        return "{}{}".format(
            self.rt_ifp.if_name.dereference(), self.rt_ifp.if_unit
        )
    ...
    def source_ip(self):
        try:
            ...
            node = obj.Object(
                "radix_node", offset=self.rt_nodes.obj_offset, vm=self.obj_vm
            )

        return node.rn_u.rn_leaf.rn_Key.dereference_as(
            "sockaddr"
        ).get_address()
...
class zone(obj.CType):
    ...
    def get_free_elements(self, elem_type):
        ...
        nxt = obj.Object(
            "zone_free_element", offset=self.free_elements, vm=self.obj_vm
        )
...
class OSString(obj.CType):
    def __str__(self):
        ...
        string_object = obj.Object(
            "String", offset=self.string, vm=self.obj_vm, length=self.length
        )
...
class vm_map_entry(obj.CType):
    ...
    def get_vnode(self):
        ...
        while (
            vnode_object.shadow.dereference() != None
            and vnode_object.v() not in seen
        ):
        ...
        if ops == self.obj_vm.profile.get_symbol("_vnode_pager_ops"):
            vpager = obj.Object(
                "vnode_pager", offset=vnode_object.pager, vm=self.obj_vm
            )
    ...
    def resident_count(self):
        ...
        if hasattr(vmobj, "all_reusable"):
            if vmobj.all_reusable == 1:
                ...
                count = (
                    vmobj.resident_page_count.v()
                    - vmobj.reusable_page_count.v()
                )
...
class sockaddr(obj.CType):
    def get_address(self):
        ...
        if family == 2:  # AF_INET
            addr_in = obj.Object(
                "sockaddr_in", offset=self.obj_offset, vm=self.obj_vm
            )
            ip = addr_in.sin_addr.s_addr.v()

        elif family == 30:  # AF_INET6
            addr_in6 = obj.Object(
                "sockaddr_in6", offset=self.obj_offset, vm=self.obj_vm
            )
            ip = addr_in6.sin6_addr.__u6_addr.v()

        elif family == 18:  # AF_LINK
            addr_dl = obj.Object(
                "sockaddr_dl", offset=self.obj_offset, vm=self.obj_vm
            )
            ip = addr_dl.v()
...
def MacProfileFactory(profpkg):
    ...
    for f in profpkg.filelist:
        if 'symbol.dsymutil' in f.filename.lower():
            memmodel, sysmap, typemap = parse_dsymutil(
    ...
    class AbstractMacProfile(obj.Profile):
        ...
        def _get_symbol_by_address_type(
            self, module, wanted_sym_address, wanted_sym_type
        ):
            ret = ""
            ...
            for (addr, (name, sym_types)) in list(mod.items()):
                for sym_type in sym_types:
                    ...
                    if (
                        wanted_sym_address == addr
                        or wanted_sym_address == self.shift_address + addr
                    ) and wanted_sym_type == sym_type:
                        ret = name
                        break
        ...
        def get_symbol_by_address_type(self, module, sym_address, sym_type):
            ...
            if key in self.sbat_cache:
                ...
                ret = self._get_symbol_by_address_type(
                    module, sym_address, sym_type
                )
    ...
    for path, _, files in os.walk(path):
        for fn in files:
            if zipfile.is_zipfile(os.path.join(path, fn)):
                new_classes.append(
                    MacProfileFactory(zipfile.ZipFile(os.path.join(path, fn)))
                )

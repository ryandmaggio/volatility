...
class macho(obj.CType):
    ...
    def _make_macho_obj(self, offset, vm):
        if self.size_cache == 32:
            self.macho_obj = obj.Object(
                self.name32, offset=offset, vm=vm, parent=self
            )
        elif self.size_cache == 64:
            self.macho_obj = obj.Object(
                self.name64, offset=offset, vm=vm, parent=self
            )
        else:
            self.macho_obj = None
...
class macho_header(macho):
    ...
    def load_commands(self):
        ...
        for i in range(self.ncmds):
            cmd = obj.Object(
                tname, offset=arr_start + offset, vm=self.obj_vm, parent=self
            )
    ...
    def load_command_of_type(self, cmd_type):
        ...
        if cmds and len(cmds) > 1:
            debug.error(
                "load_command_of_type: Multiple commands of type %d found!"
                % cmd_type
            )
        elif cmds:
            ret = cmds[0]
    ...
    def get_indirect_syms(self):
        ...
        indirect_table_addr = (
            self.link_edit_bias + self.cached_dysymtab.indirectsymoff
        )
        ...
        for idx in symtab_idxs:
            ...
            sym = obj.Object(
                "macho_nlist", offset=sym_addr, vm=self.obj_vm, parent=self
            )
            if sym.is_valid():
                syms.append(sym)
            ...
            if str(seg.segname) == "__DATA":
                for sect in self.sections_for_segment(seg):
                    if str(sect.sectname) == "__la_symbol_ptr":
                        ...
                        sym_ptr_arr = obj.Object(
                            theType="Array",
                            targetType=idx_type,
                            count=num_idxs,
                            offset=self.obj_offset + sect.offset,
                            vm=self.obj_vm,
                        )
    ...
    def segments(self):
        ...
        for load_command in load_commands:
            segment = obj.Object(
                ...
                offset=load_command.obj_offset,
                vm=self.obj_vm,
                parent=self,
            )
    ...
    def sections_for_segment(self, segment):
        ...
        for i in range(cnt):
            ...
            sect = obj.Object(
                "macho_section", offset=sect_addr, vm=self.obj_vm, parent=self
            )

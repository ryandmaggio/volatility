    ...
    '_IMAGE_RESOURCE_DIRECTORY': [
        0x12,
        {
            ...
            'Entries': [
                ...
                [
                    'array',
                    lambda x: x.NamedEntriesCount + x.IdEntriesCount,
...
class _IMAGE_EXPORT_DIRECTORY(obj.CType):
    ...
    def valid(self, nt_header):
        ...
        try:
            return (
                self.AddressOfFunctions < nt_header.OptionalHeader.SizeOfImage
                and self.AddressOfNameOrdinals
                < nt_header.OptionalHeader.SizeOfImage
                and self.AddressOfNames < nt_header.OptionalHeader.SizeOfImage
                and self.NumberOfFunctions < 0x7FFF
                and self.NumberOfNames < 0x7FFF
            )
    ...
    def _exported_functions(self):
        ...
        address_of_functions = obj.Object(
            'Array',
        ...
        address_of_name_ordinals = obj.Object(
            'Array',
        ...
        for i in range(self.NumberOfNames):
            ...
            if (
                func_rva >= exp_dir.VirtualAddress
                and func_rva < exp_dir.VirtualAddress + exp_dir.Size
            ):
        ...
        for i in range(self.NumberOfFunctions):
            ...
            if ordinal not in seen_ordinals:
                ...
                yield ordinal, func_rva, obj.NoneObject(
                    "Name RVA not accessible"
                )
    ...
    def valid(self, nt_header):
        ...
        try:
            return (
                ...
                and self.OriginalFirstThunk
                < nt_header.OptionalHeader.SizeOfImage
                and self.FirstThunk != 0
                and self.FirstThunk < nt_header.OptionalHeader.SizeOfImage
                and self.Name < nt_header.OptionalHeader.SizeOfImage
            )
        except obj.InvalidOffsetError:
            return False
    ...
    def _imported_functions(self):
        ...
        while 1:
            ...
            if thunk.OrdinalBit == 1:
                ...
                iibn = obj.Object(
                    "_IMAGE_IMPORT_BY_NAME",
                    offset=self.obj_parent.DllBase + thunk.AddressOfData,
                    vm=self.obj_native_vm,
                )
                o = iibn.Hint
                n = iibn.Name
...
class _LDR_DATA_TABLE_ENTRY(obj.CType):
    ...
    def _nt_header(self):
        ...
        try:
            dos_header = obj.Object(
                "_IMAGE_DOS_HEADER", offset=self.DllBase, vm=self.obj_native_vm
            )
            ...
            return obj.NoneObject(
                "Failed initial sanity checks. Try -u or --unsafe"
            )
    ...
    def imports(self):
        ...
        desc_size = self.obj_vm.profile.get_obj_size(
            '_IMAGE_IMPORT_DESCRIPTOR'
        )
...
class _IMAGE_DOS_HEADER(obj.CType):
    ...
    def get_version_info(self):
        ...
        for sect in nt_header.get_sections(unsafe):
            if str(sect.Name) == '.rsrc':
                root = obj.Object(
                    "_IMAGE_RESOURCE_DIRECTORY",
                    self.obj_offset + sect.VirtualAddress,
                    self.obj_vm,
                )
    ...
    def _get_image_mem(self, unsafe, fix):
        ...
        offset, data = self.get_code(
            self.obj_offset, nt_header.OptionalHeader.SizeOfImage, 0
        )
        if fix:
        ...
        for sect in nt_header.get_sections(unsafe):
            if prevsect is not None:
                sect_sizes.append(
                    sect.VirtualAddress - prevsect.VirtualAddress
                )
            prevsect = sect
        if prevsect is not None:
            sect_sizes.append(
                self.round(prevsect.Misc.VirtualSize, sa, up=True)
            )

        counter = 0
        start_addr = nt_header.FileHeader.SizeOfOptionalHeader + (
            nt_header.OptionalHeader.obj_offset - self.obj_offset
        )
        for sect in nt_header.get_sections(unsafe):
...
class _IMAGE_NT_HEADERS64(obj.CType):
    ...
    def OptionalHeader(self):
        ...
        if (
            self.obj_vm.profile.get_obj_size("address") == 8
            and self.FileHeader.Machine == 0x014C
        ):  # 32bit exe
...
class _IMAGE_NT_HEADERS(obj.CType):
    ...
    def get_sections(self, unsafe=False):
        ...
        start_addr = (
            self.FileHeader.SizeOfOptionalHeader
            + self.OptionalHeader.obj_offset
        )
...
class VerStruct(obj.CType):
            ...
            for n in self.Key:
                ...
                if n == 0:
                    ...
                    name = (
                        self.obj_vm.read(
                            self.Key.obj_offset,
                            n.obj_offset - self.Key.obj_offset,
                        )
                        .decode("utf16", "ignore")
    ...
    def display_unicode(self, string):
        ...
        if string is None:
            return ''
        return string.decode("utf16", "ignore").encode(
...
class _VS_FIXEDFILEINFO(obj.CType):
    ...
    def flags(self):
        ...
        addr_space = addrspace.BufferAddressSpace(
            self.obj_vm.get_config(), 0, data
        )
        bitmap = {
            'Debug': 0,
            'Prerelease': 1,
...
class _IMAGE_RESOURCE_DIR_STRING_U(obj.CType):
    ...
    def v(self):
        ...
        try:
            ...
            return data.decode("utf16", "ignore").encode(
                "ascii", 'backslashreplace'
            )
...
class _IMAGE_RESOURCE_DIRECTORY(obj.CType):
    ...
    def __init__(
        self, theType=None, offset=None, vm=None, parent=None, *args, **kwargs
    ):
        ...
        for irde in self.Entries:
            if irde != None:
                if irde.Name & 0x80000000:
                    ...
                    name = obj.Object(
                        "_IMAGE_RESOURCE_DIR_STRING_U",
                        (irde.Name & 0x7FFFFFFF) + self.sectoffset,
                        vm=self.obj_vm,
                        parent=irde,
                    )
                else:
                ...
                if irde.DataOffset & 0x80000000:
                    ...
                    retobj = obj.Object(
                        "_IMAGE_RESOURCE_DIRECTORY",
                        (irde.DataOffset & 0x7FFFFFFF) + self.sectoffset,
                        vm=self.obj_vm,
                        parent=irde,
                    )
                    ...
                    retobj = obj.Object(
                        "_IMAGE_RESOURCE_DATA_ENTRY",
                        irde.DataOffset + self.sectoffset,
                        vm=self.obj_vm,
                        parent=irde,
                    )

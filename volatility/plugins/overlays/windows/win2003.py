...
class EThreadCreateTime(obj.ProfileModification):
    ...
    def check(self, profile):
        ...
        return (
            ...
            and (
                (m.get('major', 0) == 5 and m.get('minor', 0) >= 2)
                or m.get('major', 0) >= 6
            )
            and profile.__class__.__name__ != 'Win2003SP0x86'
        )

    def modification(self, profile):
        overlay = {
            '_ETHREAD': [None, {'CreateTime': [None, ['WinTimeStamp', {}]]}]
        }
        profile.merge_overlay(overlay)
...
class Win2003SP0x86(obj.Profile):
    ...
    _md_vtype_module = (
        'volatility.plugins.overlays.windows.win2003_sp0_x86_vtypes'
    )
...
class Win2003SP1x86(obj.Profile):
    ...
    _md_memory_model = '32bit'
    _md_vtype_module = (
        'volatility.plugins.overlays.windows.win2003_sp1_x86_vtypes'
    )
    _md_product = ["NtProductLanManNt", "NtProductServer"]
...
class Win2003SP2x86(obj.Profile):
    ...
    _md_memory_model = '32bit'
    _md_vtype_module = (
        'volatility.plugins.overlays.windows.win2003_sp2_x86_vtypes'
    )
    _md_product = ["NtProductLanManNt", "NtProductServer"]
...
class Win2003SP1x64(obj.Profile):
    ...
    _md_build = 3790
    _md_vtype_module = (
        'volatility.plugins.overlays.windows.win2003_sp1_x64_vtypes'
    )
    _md_product = ["NtProductLanManNt", "NtProductServer"]
...
class Win2003SP2x64(obj.Profile):
    ...
    _md_build = 3791
    _md_vtype_module = (
        'volatility.plugins.overlays.windows.win2003_sp2_x64_vtypes'
    )
    _md_product = ["NtProductLanManNt", "NtProductServer"]

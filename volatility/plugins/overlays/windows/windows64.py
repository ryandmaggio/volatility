...
class LIST_ENTRY32(windows._LIST_ENTRY):
    ...
    def get_next_entry(self, member):
        return obj.Object(
            "LIST_ENTRY32", offset=self.m(member).v(), vm=self.obj_vm
        )
...
class Windows64Overlay(obj.ProfileModification):
    ...
    def modification(self, profile):
        profile.merge_overlay(
            {
                'VOLATILITY_MAGIC': [
                    0x0,
                    {
                        'PoolAlignment': [
                            0x0,
                            ['VolatilityMagic', dict(value=16)],
                        ],
                        'KUSER_SHARED_DATA': [
                            0x0,
                            [
                                'VolatilityMagic',
                                dict(value=0xFFFFF78000000000),
                            ],
                        ],
                    },
                ]
        ...
        profile.vtypes["_IMAGE_NT_HEADERS"] = profile.vtypes[
            "_IMAGE_NT_HEADERS64"
        ]
...
class WinPeb32(obj.ProfileModification):
    ...
    def modification(self, profile):
        ...
        for prof in list(profiles.values()):
            if (
                prof._md_os == "windows"
                and prof._md_major == meta.get("major")
                and prof._md_minor == meta.get("minor")
                and (
                    (prof._md_build == meta.get("build"))
                    or (prof._md_build + 1 == meta.get("build"))
                )
                and prof._md_memory_model == "32bit"
            ):
        ...
        if profile_32bit == None:
            debug.warning(
                "Cannot find a 32-bit equivalent profile. The "
        ...
        profile.vtypes.update(
            {
                "_PEB32_LDR_DATA": self.cast_as_32bit(
                    profile_32bit.vtypes["_PEB_LDR_DATA"]
                ),
                "_LDR32_DATA_TABLE_ENTRY": self.cast_as_32bit(
                    profile_32bit.vtypes["_LDR_DATA_TABLE_ENTRY"]
                ),
                '_UNICODE32_STRING': self.cast_as_32bit(
                    profile_32bit.vtypes["_UNICODE_STRING"]
                ),
            }
        )

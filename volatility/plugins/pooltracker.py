...
class PoolTracker(common.AbstractWindowsCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        config.add_option(
            'TAGFILE',
            short_option='T',
            help='Pool tag file (pooltag.txt)',
            default=None,
        )
        config.add_option(
            'WHITELIST',
    ...
    def calculate(self):
        ...
        table_size = obj.Object(
            "address",
            offset=track_table - kernel_space.profile.get_obj_size("address"),
            vm=kernel_space,
        )
        ...
        if (
            not kernel_space.is_valid_address(track_table)
        ...
        for pool, buf in data:
            ...
            outfd.write(
                "Pool Header: {0:#x}, Size: {1}\n".format(
                    pool.obj_offset, pool.BlockSize * pool_alignment
                )
            )
            outfd.write(
                "{0}\n".format(

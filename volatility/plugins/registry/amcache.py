...
class AmCache(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        if not self._config.HIVE_OFFSET:
            ...
            name = obj.Object(
                "_CMHIVE", vm=addr_space, offset=self._config.HIVE_OFFSET
            ).get_name()
        ...
        for key, name in self.regapi.reg_yield_key(None, filekey):
            for guidkey in self.regapi.reg_get_all_subkeys(
                None, None, given_root=key
            ):
                result = {}
                for thefile in self.regapi.reg_get_all_subkeys(
                    None, None, given_root=guidkey
                ):
                    result["hive"] = name
                    for vname, value in self.regapi.reg_yield_values(
                        None, None, thetype=None, given_root=thefile
                    ):
                        result["valuename"] = vname
                        result["value"] = value
                        result["key"] = thefile
                        result["description"] = fileitems.get(
                            str(vname), "UNKNOWN"
                        )
                        result["timestamp"] = ""
                        if str(vname) in ["11", "12", "17"]:
                            try:
                                bufferas = addrspace.BufferAddressSpace(
                                    self._config, data=struct.pack("<Q", value)
                                )
                                result["timestamp"] = obj.Object(
                                    "WinTimeStamp",
                                    vm=bufferas,
                                    offset=0,
                                    is_utc=True,
        ...
        for key, name in self.regapi.reg_yield_key(None, progkey):
            for guidkey in self.regapi.reg_get_all_subkeys(
                ...
                result["hive"] = name
                for vname, value in self.regapi.reg_yield_values(
                    None, None, thetype=None, given_root=guidkey
                ):
                    result["valuename"] = vname
                    result["value"] = value
                    result["key"] = guidkey
                    result["description"] = programsitems.get(
                        str(vname), "UNKNOWN"
                    )
                    result["timestamp"] = ""
                    if str(vname) == "a":
                        try:
                            bufferas = addrspace.BufferAddressSpace(
                                self._config, data=struct.pack("<I", value)
                            )
                            result["timestamp"] = obj.Object(
                                "UnixTimeStamp",
                                vm=bufferas,

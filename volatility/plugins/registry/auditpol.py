...
class AuditPolDataXP(obj.CType):
    def __str__(self):
        ...
        msg += "Audit Dir Service Access: {0}\n\tAudit Account Logon Events: {1}\n".format(
            self.DirectoryAccess, self.AccountLogon
        )
        return msg
...
class AuditPolDataVista(obj.CType):
    def __str__(self):
        ...
        msg += "Authorization Policy Change: {0}\n\tMPSSVC Rule: {1}\n\tFiltering Platform Policy Change: {2}\n\t".format(
            ...
            self.MPSSVCRule,
            self.FilteringPlatformPolicyChange,
        )
        ...
        msg += "Directory Service Access: {0}\n\tDirectory Service Changes: {1}\n\tDirectory Service Replication: {2}\n\t".format(
            self.DirectoryServiceAccess,
            self.DirectoryServiceChange,
            self.DirectoryServiceReplication,
        )
        ...
        msg += "Kerberos Service Ticket Operations: {0}\n\tOther Account Logon Events: {1}\n\tKerberos Authentication Service: {2}\n".format(
            self.KerberosOperations,
            self.AccountLogonOther,
            self.KerberosAuthentication,
        )
...
class Auditpol(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        for value, data_raw in regapi.reg_yield_values(
            'security', 'Policy\\PolAdtEv', thetype='REG_NONE'
        ):
            bufferas = addrspace.BufferAddressSpace(
                self._config, data=data_raw
            )
            if version <= (5, 1):
    ...
    def unified_output(self, data):
        return TreeGrid([("Item", str), ("Detail", str)], self.generator(data))
            ...
            if self._config.HEX:
                ...
                raw = "\n".join(
                    [
                        ...
                        for o, h, c in utils.Hexdump(data_raw)
                    ]
                )
                print(raw)

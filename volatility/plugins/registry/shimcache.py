...
shimrecs_type_xp = {
    'ShimRecords': [
        None,
        {
            ...
            'Entries': [
                0x190,
                ['array', lambda x: x.NumRecords, ['AppCompatCacheEntry']],
            ],
        },
    ],
}
...
class ShimCache(common.AbstractWindowsCommand):
    ...
    def remove_unprintable(item):
        return ''.join(
            [
                str(c)
                for c in item
                if (ord(c) > 31 or ord(c) == 9) and ord(c) <= 126
            ]
        )
    ...
    def get_entries(addr_space, regapi):
        ...
        if shimdata.Magic not in [0xDEADBEEF, 0xBADC0FFE, 0xBADC0FEE]:
            debug.warning(
                "ShimRecords.Magic value {0:X} is not valid".format(
                    shimdata.Magic
                )
            )
            return  # previously raise StopIteration
        ...
        for e in shimdata.Entries:
            if xp:
                ...
                yield ShimCache.remove_unprintable(
                    bufferas.read(int(e.PathOffset), int(e.Length))
                ), e.LastModified, None

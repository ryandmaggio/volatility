...
class UserAssist(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        win7 = (
            addr_space.profile.metadata.get('major', 0) == 6
    ...
    def parse_data_dict(self, dat_raw):
        ...
        if (
            len(dat_raw)
            < bufferas.profile.get_obj_size('_VOLUSER_ASSIST_TYPES')
            or uadata == None
        ):
            return None
    ...
    def parse_data(self, dat_raw):
        ...
        if hasattr(uadata, "FocusCount"):
            ...
            output += "\n{0:15} {1}\n{2:15} {3}".format(
                "Focus Count:", uadata.FocusCount, "Time Focused:", time
            )
        output += f"\n{'Last updated:':15} {uadata.LastUpdated}\n"
        return output
    ...
    def generator(self, data):
        ...
        for win7, reg, key in data:
            if key:
                keyfound = True
                for s in self.regapi.reg_get_all_subkeys(
                    None, None, given_root=key
                ):
                    if s.Name == None:

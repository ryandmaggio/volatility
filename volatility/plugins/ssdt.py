...
def find_tables(nt_base, start_addr, vm):
    ...
    if use_distorm:
        ...
        for op in distorm3.DecomposeGenerator(
            start_addr, data, distorm3.Decode64Bits
        ):
            ...
            if (
                op.mnemonic == 'CMP'
                and op.operands[0].dispSize == 32
                and op.operands[0].value == 0
            ):
                if op.size == 9:
                    service_tables.append(nt_base + op.operands[0].disp)
                elif op.size in [7, 8]:
                    service_tables.append(
                        op.address + op.size + op.operands[0].disp
                    )
            elif (
                op.mnemonic == 'LEA'
                and op.size == 7
                and op.operands[1].dispSize == 32
                and op.operands[1].disp > 0
            ):
...
class SSDT(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        if addr_space.profile.metadata.get('memory_model', '32bit') == '32bit':
            ...
            for table_addr in find_tables(
                ...
                ssdt_obj = obj.Object(
                    "_SERVICE_DESCRIPTOR_TABLE", table_addr, addr_space
                )
                ssdts.add(ssdt_obj)
        ...
        for ssdt_obj in ssdts:
            for i, desc in enumerate(ssdt_obj.Descriptors):
                ...
                if (
                    not desc.is_valid()
                    or desc.ServiceLimit <= 0
                    or desc.ServiceLimit >= 2048
                    or desc.KiServiceTable <= 0x80000000
                ):
                    break
                else:
                    tables.add(
    ...
    def generator(self, data):
        ...
        bits32 = (
            addr_space.profile.metadata.get('memory_model', '32bit') == '32bit'
        )

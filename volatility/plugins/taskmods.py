...
class DllList(common.AbstractWindowsCommand, cache.Testable):
    ...
    def virtual_process_from_physical_offset(addr_space, offset):
        ...
        flat_addr_space = utils.load_as(
            addr_space.get_config(), astype='physical'
        )
        ...
        tleoffset = addr_space.profile.get_obj_offset(
        ...
        for ofs in offsets:
            ethread = obj.Object(
                "_ETHREAD",
                offset=flateproc.ThreadListHead.Flink.v() - ofs,
                vm=addr_space,
            )
            ...
            if virtual_process and offset == addr_space.vtop(
                virtual_process.obj_offset
            ):
                return virtual_process

        return obj.NoneObject(
            "Unable to bounce back from virtual _ETHREAD to virtual _EPROCESS"
        )

...
class TrueCryptPassphrase(common.AbstractWindowsCommand):
    ...
    def scan_module(addr_space, module_base, min_length):
        ...
        dos_header = obj.Object(
            "_IMAGE_DOS_HEADER", offset=module_base, vm=addr_space
        )
...
class TrueCryptSummary(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        if memory_model == '32bit':
            ...
            for subkey in regapi.reg_get_all_subkeys(None, key=x86key):
                if str(subkey.Name) == "TrueCrypt":
                    ...
                    version = regapi.reg_get_value(
                        "software", key=subpath, value="DisplayVersion"
                    )
                    if version:
                        yield "Registry Version", "{0} Version {1}".format(
                            str(subkey.Name), version
                        )
        ...
        for proc in tasks.pslist(addr_space):
            if str(proc.ImageFileName).lower() == "truecrypt.exe":
                yield "Process", "{0} at {1:#x} pid {2}".format(
                    proc.ImageFileName, proc.obj_offset, proc.UniqueProcessId
                )
        ...
        for mod in modules.lsmod(addr_space):
            ...
            if basename.endswith("truecrypt.sys") or fullname.endswith(
                "truecrypt.sys"
            ):
                yield "Kernel Module", "{0} at {1:#x} - {2:#x}".format(
                    mod.BaseDllName, mod.DllBase, mod.DllBase + mod.SizeOfImage
                )
        ...
        for driver in scanner.calculate():
            ...
            if drivername.endswith("truecrypt") or servicekey.endswith(
                "truecrypt"
            ):
...
class TrueCryptMaster(common.AbstractWindowsCommand):
            ...
            addr_space.profile.merge_overlay(
                {
                    'EXTENSION': [
                        None,
                        {
                            'wszVolume': [
                                None,
                                ['String', dict(length=260, encoding="utf16")],
                            ],
                        },
                    ],
                    'CRYPTO_INFO_t': [
    ...
    def render_text(self, outfd, data):
        for device in data:
            ...
            outfd.write(
                "Removable: {0}\n".format(
                    "Yes" if ext.bRemovable == 1 else "No"
                )
            )
            outfd.write(
                "Read Only: {0}\n".format(
                    "Yes" if ext.bReadOnly == 1 else "No"
                )
            )
            ...
            key = device.obj_vm.read(
                ext.cryptoInfo.master_keydata.obj_offset, 64
            )
            ...
            if self._config.DUMP_DIR:
                if not os.path.isdir(self._config.DUMP_DIR):
                    debug.error(
                        "The path {0} is not a valid directory".format(

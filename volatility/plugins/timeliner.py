        ...
        overlay = {
            ...
            '_IMAGE_DEBUG_DIRECTORY': [
                None,
                {
                    'TimeDateStamp': [
                        None,
                        ['UnixTimeStamp', dict(is_utc=True)],
                    ],
                },
            ],
        }
...
class TimeLiner(common.AbstractWindowsCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        config.add_option(
            'HIVE',
            short_option='H',
            help='Gather Timestamps from a Particular Registry Hive',
            type='str',
        )
        config.add_option(
            'USER',
            short_option='U',
            help='Gather Timestamps from a Particular User\'s Hive(s)',
            type='str',
        )
        config.add_option(
            "MACHINE",
        ...
        config.add_option(
            "TYPE",
            default="".join([",".join(x for x in sorted(self.types))]),
            help="Type of artifact to use in timeline (default is all, but \"Registry\")",
        )

    def unified_output(self, data):
        return TreeGrid(
            [("Start", str), ("Header", str), ("Item", str), ("Details", str)],
            self.generator(data),
        )
    ...
    def calculate(self):
        if (
            self._config.HIVE or self._config.USER
        ) and "Registry" not in self._config.TYPE:
            debug.error(
        ...
        for offset, eprocess, ps_sources in psx:
            ...
            if "Process" in self._config.TYPE:
                ...
                yield self.getoutput(
                    line, eprocess.CreateTime, end=eprocess.ExitTime, body=body
                )
            ...
            if (
                eprocess.ObjectTable.HandleTableList
            ...
            if eprocess.IsWow64 and "TimeDateStamp" in self._config.TYPE:
                for vad, address_space in eprocess.get_vads(
                    ...
                    if vad.FileObject.FileName:
                        ...
                        try:
                            pe_file = obj.Object(
                                "_IMAGE_DOS_HEADER", offset=0, vm=bufferas
                            )
                        ...
                        yield self.getoutput(
                            line, header.FileHeader.TimeDateStamp, body=body
                        )

            # get DLL PE timestamps
            mods = dict()
            if (
                "TimeDateStamp" in self._config.TYPE
                or "LoadTime" in self._config.TYPE
            ):
                mods = dict(
                    (mod.DllBase.v(), mod)
        ...
        if version[0] == 5:
            ...
            if "Socket" in self._config.TYPE:
                socks = sockscan.SockScan(
                    self._config
                ).calculate()  # you can use sockscan instead if you uncomment
            for sock in socks:
        ...
        for thread in threads:
            ...
            yield self.getoutput(
                line, thread.CreateTime, end=thread.ExitTime, body=body
            )
        ...
        for aspace, procs, mod_base, mod_name in data:
            ...
            if space != None:
                try:
                    pe_file = obj.Object(
                        "_IMAGE_DOS_HEADER", offset=mod_base, vm=space
                    )
                    header = pe_file.get_nt_header()
                except ValueError as ve:
                    continue
                    ...
                    if (
                        ...
                        if hasattr(uadata, "Count"):
                            ...
                            count = "{0}".format(
                                ...
                                else uadata.CountStartingAtFive - 5
                            )
        ...
        for path, lm, lu in shimdata:
            line = "[{2}SHIMCACHE]{0} {1}{0} ".format(
                "" if body else "|", path, self._config.MACHINE
            )
            if lu:
        ...
        if (
            "_HBASE_BLOCK" in self._config.TYPE
            ...
            for o in regapi.all_offsets:
                if "_HBASE_BLOCK" in self._config.TYPE:
                    ...
                    yield self.getoutput(
                        line, h.BaseBlock.TimeStamp, body=body
                    )

                if (
                    "_CMHIVE" in self._config.TYPE
                    and version[0] == 6
                    and addr_space.profile.metadata.get('build', 0) >= 7601
                ):
                    line = (
                        line
                    ) = "[{2}_CMHIVE LastWriteTime]{0} {1}{0} ".format(
                        "" if body else "|",
                        regapi.all_offsets[o],
                        self._config.MACHINE,
                    )
        ...
        if "Registry" in self._config.TYPE:
            ...
            regdata = regapi.reg_get_all_keys(
                self._config.HIVE, self._config.USER, reg=True, rawtime=True
            )
        ...
        if "Timer" in self._config.TYPE:
            ...
            interrupt = (
                KUSER_SHARED_DATA.InterruptTime.High1Time << 32
            ...
            for timer, module in data:
                ...
                try:
                    ...
                    due_time = obj.Object(
                        "WinTimeStamp", is_utc=True, offset=0, vm=bufferas
                    )
                except TypeError:
                    due_time = 0

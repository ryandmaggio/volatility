...
class VADInfo(taskmods.DllList):
        ...
        for task in data:
            for vad in task.VadRoot.traverse():
                if self._config.ADDR is not None and (
                    self._config.ADDR < vad.Start
                    or self._config.ADDR > vad.End
                ):
                    continue
                if vad != None:
                    ...
                    protection = PROTECT_FLAGS.get(
                        vad.VadFlags.Protection.v(),
                        hex(vad.VadFlags.Protection),
                    )

                    # translate the vad type if its available (> XP)
                    if hasattr(vad.VadFlags, "VadType"):
                        vadType = MI_VAD_TYPE.get(
                            vad.VadFlags.VadType.v(), hex(vad.VadFlags.VadType)
                        )
    ...
    def write_vad_short(self, outfd, vad):
        ...
        outfd.write(
            "Protection: {0}\n".format(
                PROTECT_FLAGS.get(
                    vad.VadFlags.Protection.v(), hex(vad.VadFlags.Protection)
                )
            )
        )
        # translate the vad type if its available (> XP)
    ...
    def write_vad_control(self, outfd, vad):
        ...
        outfd.write(
            "NumberOfSectionReferences: {0:10} NumberOfPfnReferences:  {1:10}\n".format(
                control_area.NumberOfSectionReferences,
                control_area.NumberOfPfnReferences,
            )
        )
        outfd.write(
            "NumberOfMappedViews:       {0:10} NumberOfUserReferences: {1:10}\n".format(
                control_area.NumberOfMappedViews,
                control_area.NumberOfUserReferences,
            )
        )
        ...
        if file_object:
            outfd.write(
                "FileObject @{0:08x}, Name: {1}\n".format(
                    file_object.obj_offset,
                    str(file_object.file_name_with_device() or ''),
                )
            )
    ...
    def write_vad_ext(self, outfd, vad):
        ...
        outfd.write(
            "First prototype PTE: {0:08x} Last contiguous PTE: {1:08x}\n".format(
                    ...
                    if vad.Parent:
                        outfd.write(
                            "vad_{0:08x} -> vad_{1:08x}\n".format(
                                vad.Parent.obj_offset or 0, vad.obj_offset
                            )
                        )
                        fillcolor = "white"
...
class VADDump(VADInfo):
    ...
    def render_text(self, outfd, data):
        ...
        for task in data:
            ...
            for vad, _addrspace in task.get_vads(
                vad_filter=filter, skip_max_commit=True
            ):

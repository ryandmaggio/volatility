    ...
    def calculate(self):
            ...
            try:
                if self._config.IGNORE_CASE:
                    module_pattern = re.compile(
                        self._config.REGEX,
                        flags=sre_constants.SRE_FLAG_IGNORECASE,
                    )
                else:
        ...
        if self._config.OFFSET is not None:
            if not addr_space.is_valid_address(self._config.OFFSET):
                debug.error(
                    "Specified offset is not valid for the provided address space"
                )
            pefile = obj.Object(
                "_IMAGE_DOS_HEADER", self._config.OFFSET, addr_space
            )
            if pefile.is_valid():
                yield None, pefile
        ...
        for task in tasks:
            ...
            for module in task.get_load_modules():
                if self._config.REGEX is not None:
                    if not (
                        module_pattern.search(str(module.FullDllName))
                        or module_pattern.search(str(module.BaseDllName))
                    ):
                        continue
                pefile = obj.Object(
                    "_IMAGE_DOS_HEADER", module.DllBase, process_space
                )
                if pefile.is_valid():
                    yield module, pefile
    ...
    def render_text(self, outfd, data):
        ...
        for module, pefile in data:
            ...
            if vinfo != None:
                ...
                outfd.write(
                    "  File Type       : {0}\n".format(
                        vinfo.FileInfo.file_type()
                    )
                )
                outfd.write(
                    "  File Date       : {0}\n".format(
                        vinfo.FileInfo.FileDate or ''
                    )
                )
                for string, value in vinfo.get_file_strings():
                    outfd.write("  {0} : {1}\n".format(string, value))

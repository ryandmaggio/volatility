...
class MultiScanInterface(object):
    ...
    def __init__(
        ...
        self.pool_alignment = obj.VolMagic(
            self.address_space
        ).PoolAlignment.v()
    ...
    def _check_pool_type(self, check, pool_header):
        ...
        return (
            (non_paged and pool_header.NonPagedPool)
            or (free and pool_header.FreePool)
            or (paged and pool_header.PagedPool)
        )
    ...
    def _run_all_checks(self, checks, pool_header):
        ...
        for check, args in checks:
            if check == "CheckPoolSize":
                ...
                custom_check = registry.get_plugin_classes(scan.ScannerCheck)[
                    check
                ](pool_header.obj_vm, **args)
    ...
    def scan(self):
        ...
        scanner = MultiPoolScanner(
            needles=[scanner.pooltag for scanner in scanners]
        )

        pool_tag_offset = space.profile.get_obj_offset(
            "_POOL_HEADER", "PoolTag"
        )

        for tag, offset in scanner.scan(
            address_space=space,
            ...
            if not self._run_all_checks(
                checks=scanobj.checks, pool_header=pool
            ):
                continue
            ...
            result = pool.get_object(
                struct_name=scanobj.struct_name,
...
class CheckPoolType(scan.ScannerCheck):
    ...
    def __init__(
        self, address_space, paged=False, non_paged=False, free=False, **kwargs
    ):
        scan.ScannerCheck.__init__(self, address_space, **kwargs)
    ...
    def check(self, offset):
        pool_hdr = obj.Object(
            '_POOL_HEADER', vm=self.address_space, offset=offset - 4
        )

        return (
            (self.non_paged and pool_hdr.NonPagedPool)
            or (self.free and pool_hdr.FreePool)
            or (self.paged and pool_hdr.PagedPool)
        )
...
class SinglePoolScanner(scan.BaseScanner):
    def object_offset(self, found, address_space):
        ...
        ## for the PoolTag.
        return found - self.buffer.profile.get_obj_offset(
            '_POOL_HEADER', 'PoolTag'
        )

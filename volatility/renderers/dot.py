...
class DotRenderer(Renderer):
    ...
    def description(self, node):
        ...
        for column in self._columns:
            text = self._text_cell_renderers[column.index].render(
    ...
    def render(self, outfd, grid):
        ...
        self._text_cell_renderers = self._text_cell_renderers_func(
            self._columns
        )

        if grid.max_depth() <= 1:
            debug.warning(
                "Dot output will be unhelpful since the TreeGrid is a flat list"
            )
        outfd.write(
            "digraph output {\n  node[shape = Mrecord];\n  # rankdir=LR;\n"
        )

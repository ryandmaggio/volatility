...
class SqliteRenderer(Renderer):
    ...
    def render(self, outfd, grid):
        if not self._config.OUTPUT_FILE:
            debug.error(
                "Please specify a valid output file using --output-file"
            )

        self._db = sqlite3.connect(
            self._config.OUTPUT_FILE, isolation_level=None
        )
        ...
        create = (
            ...
            + ", ".join(
                [
                    '"'
                    + self._sanitize_name(i.name)
                    + '" '
                    + self._column_type(i.type)
        ...
        def _add_multiple_row(node, accumulator):
            ...
            accumulator[1].append(
                [accumulator[0]] + [str(v) for v in node.values]
            )
        ...
        if len(self._accumulator[1]) > 0:
            ...
            insert = (
                ...
                + ") "
                + " VALUES (?, "
                + ", ".join(["?"] * (len(self._accumulator[1][0]) - 1))
                + ")"
            )

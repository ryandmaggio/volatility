...
def get_kdbg(addr_space):
    ...
    if obj.VolMagic(addr_space).KPCR.value:
        kpcr = obj.Object(
            "_KPCR", offset=obj.VolMagic(addr_space).KPCR.value, vm=addr_space
        )
        kdbg = kpcr.get_kdbg()
        if kdbg.is_valid():
            return kdbg
...
def find_module(modlist, mod_addrs, addr):
    ...
    if (
        mod.obj_vm.address_compare(addr, mod.DllBase) != -1
        and mod.obj_vm.address_compare(addr, mod.DllBase + mod.SizeOfImage)
        == -1
    ):
        return mod
    else:
